...
//     procedure Push(ATableSpecial: TXLSTableSpecialSpecifier; ACol1,ARow1,ACol2,ARow2: integer); overload;
     ...
     procedure DoValue;
     procedure DoBoolean(AArgCount: integer; AOpAND: boolean);
...
begin
  Dec(FStackPtr);
  if FStackPtr < -1 then
    raise Exception.Create('Missing operand in expression');
end;
...
procedure TValueStack.DoBoolean(AArgCount: integer; AOpAND: boolean);
var
  i: integer;
  B: boolean;
  Res: boolean;
...
{$ifndef DELPHI_6_OR_LATER}
  raise Exception.Create('DATE not supported by Delphi 5');
{$endif}
...
{$else}
    raise Exception.Create('DATE360 not supported by Delphi 5');
{$endif}
...
{$ifdef DELPHI_6_OR_LATER}
  D := EndOfTheMonth(D);
{$else}
  raise Exception.Create('EOMONTH not supported by Delphi 5');
{$endif}
  Push(Trunc(D));
end;
...
{$ifdef DELPHI_5}
    raise Exception.Create('NETWORKDAYS not supported by Delphi 5');
{$else}
...
{$ifdef DELPHI_6_OR_LATER}
  ...
  T := IncSecond(T,SS);
{$else}
  raise Exception.Create('TIME not supported by Delphi 5');
{$endif}
...
{$else}
     raise Exception.Create('YEARFRAC not supported by Delphi 5');
{$endif}
    end;
    1: begin
      if D1 = D2 then
...
begin
  case AValue.RefType of
    ...
    xfrtAreaList: raise Exception.Create('Invalid value');
...
begin
  ...
  case AValue.RefType of
    ...
    xfrtArea,
    xfrtXArea,
    xfrtAreaList: raise Exception.Create('Invalid ref type in calc formula');
  end;
end;
...
{$ifdef _AXOLOT_DEBUG }
    if Length(FStack) > 1024 then
      raise Exception.Create('Evaluate stack have grown to horrendeous size!');
{$endif}
  end;
end;
  ...
  if E1 <> errUnknown then
    ...
    if VT1 = VT2 then begin
      case VT1 of
        ...
        xfvtError : raise Exception.Create('Invalid value type');
      end;
      FStack[FStackPtr].RefType := xfrtNone;
...
begin
  ...
  case AId of
    ...
    036: begin  // AND
      FStack.DoBoolean(AArgCount,True);
    end;
    037: begin  // OR
      FStack.DoBoolean(AArgCount,False);
    end;
    046: begin  // VAR
  ...
  for i := ArgCount - 1 downto 0 do begin
    if FuncId < 0 then
      ...
      if G_XLSExcelFuncNames[FuncId].Args.ArgCount = 255 then
        ...
        raise Exception.Create('To many arguments in function');
    end;
    FV := FStack.Pop;
...
{$ifdef DELPHI_5}
      raise Exception.Create('ROUND not supported by Delphi 5');
{$else}
...
begin
  ...
  while NativeInt(APtgs) < LastPtgs do begin
    ...
    if FIsExcel97 then
      ...
      B := APtgs.Id;
    case B of
      xptgNone    : raise Exception.Create('Illegal ptgs');
      xptgOpAdd,
      xptgOpSub,
      xptgOpMult,
      xptgOpDiv,
...
//      xptgRPar: ;
      ...
      xptgArrayFmlaChild,
      xptgDataTableFmlaChild : raise Exception.Create('This ptgs shall not be calculated');
...
{$ifdef XLS_BIFF}
      ...
      // ************** End Excel 97 *****************
{$endif}
      else raise Exception.CreateFmt('Invalid ptgs: %.2X',[APtgs.Id]);
    end;
      ...
      for C := 0 to Arr.Width - 1 do begin
        if not (AHasParentFormula and (R = 0) and (C = 0)) then begin
          ...
          case FV.ValType of
            ...
            else          raise Exception.Create('Invalid array value');
          end;
        end;
        Inc(CT);
...
begin
  ...
  if Result then begin
    case FV.RefType of
      ...
      xfrtXArea: raise Exception.Create('TODO XArea');
      xfrtArray: begin
...
begin
  ...
  case FArgs[AIndex].RefType of
    ...
    xfrtAreaList: raise Exception.Create('Argument is N/A');
    xfrtArray   : begin
      ACol1 := 0;
      ARow1 := 0;
    ...
    xfrtArrayArg: raise Exception.Create('Argument is N/A');
  end;
end;
...
begin
  ...
  case FArgs[Index].RefType of
    ...
    xfrtXArea   : raise Exception.Create('Argument is N/A');
    xfrtAreaList: raise Exception.Create('Argument is N/A');
...
begin
  ...
  case FArgs[Index].RefType of
    ...
    xfrtAreaList,
    xfrtArray,
    xfrtArrayArg: raise Exception.Create('Argument is not a single cell');
  end;
end;
