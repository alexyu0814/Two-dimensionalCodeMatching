...
begin
  ...
  for i := 0 to FManager.Workbook.PivotCaches.Count - 1 do begin
    ...
    OPC := FManager.FileData.OPC.CreatePivotCacheDefinition(Nil,FManager.Workbook.PivotCaches[i].Index + 1);

    Strm := FManager.FileData.OPC.ItemCreateStream(OPC);
    try
      FManager.Workbook.PivotCaches[i].DOM.SaveToStream(Strm);
    finally
      FManager.FileData.OPC.ItemCloseStream(OPC,Strm);
    end;

//    PivotCache.CacheId := FManager.Workbook.PivotCaches[i].CacheId;
    PivotCache.CacheId := FManager.Workbook.PivotCaches[i].Index + 1;
    PivotCache.R_Id := OPC.Id;

    OPC2 := FManager.FileData.OPC.CreatePivotCacheRecords(OPC,FManager.Workbook.PivotCaches[i].Index + 1);

    Strm := FManager.FileData.OPC.ItemCreateStream(OPC2);
    try
      FManager.Workbook.PivotCaches[i].Records.Data.SaveToStream(Strm);
    finally
...
begin
  ...
  for i := 0 to ASheet.PivotTables.Count - 1 do begin
    j := ASheet.PivotTables[i].Cache.Index + 1;

    if ASheet.PivotTables[i].DOM.Root.Count > 0 then
      ASheet.PivotTables[i].DOM.Root[0].Attributes.Update('cacheId',j);

    OPC := FManager.FileData.OPC.CreatePivotTable;

    Stream := FManager.FileData.OPC.ItemCreateStream(OPC);
    try
      ASheet.PivotTables[i].DOM.SaveToStream(Stream);
    finally
      FManager.FileData.OPC.ItemCloseStream(OPC,Stream);
    end;

    FManager.FileData.OPC.CreatePivotCacheDefinition(OPC,j);
  end;
...
var
  ...
  Reference: TCT_PivotAreaReference;
begin
  for i := 0 to ASheet.SheetViews.Count - 1 do begin
    ...
    for j := 0 to SView.PivotSelection.Count - 1 do begin
      ...
      PivotSelection.Axis := TST_Axis(PSelection.Axis);
      ...
      PivotSelection.PivotArea.Type_ := TST_PivotAreaType(PSelection.PivotArea.Type_);
      ...
      PivotSelection.PivotArea.Axis := TST_Axis(PSelection.PivotArea.Axis);
...
begin
  FManager.FileData.OPC.CreateWorkbook;
end;
