...
{$endif}
     ...
     Xc12Utils5, Xc12Graphics, Xc12Manager5,
{$ifdef _AXOLOT_BETA}
     XLSChart5, XLSBuildChart5,
{$endif}
{$ifdef XLS_BIFF}
     BIFF_DrawingObj5,
{$endif}
     XLSUtils5, XLSColumn5, XLSRow5;

type TAddImageEvent97 = function(AAnchor: TCT_TwoCellAnchor; const AColOffs,ARowOffs: double): boolean of object;

type TDrwOptImagePositioning = (doipMoveButDoNotSize,doipMoveAndSize,doipDoNotMoveOrSize);

type TXLSDrwFillType = (xdftNone,xdftSolid,xdftGradient,xdftPicture);
...
public
     ...
     property Items[const Index: integer]: TXLSDrwFillGradientStop read GetItems;
     end;
...
public
     ...
     function  LoadPicture(const APictureName: AxUCString): boolean;
...
private
     ...
     procedure SetFillType(const Value: TXLSDrwFillType);
protected
     FFillChoice: TEG_LineFillProperties;
     FFill: TXLSDrwFill;
public
     constructor Create(AFillChoice: TEG_LineFillProperties);
...
private
     function GetChart: TXPGDocXLSXChart;
protected
     FChart: TXPGDocXLSXChart;
     FName : AxUCString;
public
     constructor Create(AOwner: TXLSDrawing; AChart: TXPGDocXLSXChart; const AName: AxUCString);
     destructor Destroy; override;

     property Chart: TXPGDocXLSXChart read GetChart;
...
public
     ...
     function  Find(const AName: AxUCString): TXLSDrawingChart; overload;

     function Add(AChart: TXPGDocXLSXChart; const AName: AxUCString): TXLSDrawingChart;
...
{$ifdef _AXOLOT_BETA}
     function  DoAddChart(AAnchor: TCT_TwoCellAnchor): TXLSChart;
{$else}
...
{$ifdef _AXOLOT_BETA}
     property Charts   : TXLSCharts read FCharts;
{$else}
...
{$ifdef _AXOLOT_BETA}
function TXLSDrawing.DoAddChart(AAnchor: TCT_TwoCellAnchor): TXLSChart;
var
  Chart: TXLSChart;
  Xc12Chart: TXPGDocXLSXChart;
  Builder: TXLSBuildChart;
begin
  Xc12Chart := AAnchor.Objects.GraphicFrame.Graphic.GraphicData.Chart;
  Chart := FCharts.Add;
  Chart.Anchor := AAnchor;
  Builder := TXLSBuildChart.Create(FManager,Chart);
  try
    Builder.Build(Xc12Chart);
  finally
    Builder.Free;
  end;
  Result := Nil;
...
begin
  if (AAnchor.Objects.GraphicFrame <> Nil) and
    ...
    if (AAnchor.Objects.GraphicFrame.NvGraphicFramePr <> Nil) and
       (AAnchor.Objects.GraphicFrame.NvGraphicFramePr.CNvPr <> Nil) then
      S := AAnchor.Objects.GraphicFrame.NvGraphicFramePr.CNvPr.Name
    else
      S := '';

    Result := FCharts.Add(AAnchor.Objects.GraphicFrame.Graphic.GraphicData.Chart,S);
...
begin
  ...
  if Assigned(FAddImage97) then
    FAddImage97(Anchor,AWidth / FColumns[ACol].PixelWidth,AHeight / FRows[ARow].PixelHeight);
...
begin
  ...
  if Image <> Nil then begin
    ...
    Anchor.From.ColOff := Round(APicture97.Col1Offset);
    Anchor.From.Row := APicture97.Row1;
    Anchor.From.RowOff := Round(APicture97.Row1Offset);

    Anchor.To_.Col := APicture97.Col2;
    Anchor.To_.ColOff := Round(APicture97.Col2Offset);
    Anchor.To_.Row := APicture97.Row2;
    Anchor.To_.RowOff := Round(APicture97.Row2Offset);
...
//  Result.FAnchor.Objects.Sp.Style.A_LnRef.Color.CreateA_SchemeClr;
//  Result.FAnchor.Objects.Sp.Style.A_LnRef.Color.A_SchemeClr.Val := stscvAccent1;
//  Result.FAnchor.Objects.Sp.Style.A_LnRef.Color.A_SchemeClr.Transform.AddInteger(stcttShade,'50000');
//
//  Result.FAnchor.Objects.Sp.Style.A_FillRef.Idx := 1;
//  Result.FAnchor.Objects.Sp.Style.A_FillRef.Color.CreateA_SchemeClr;
//  Result.FAnchor.Objects.Sp.Style.A_FillRef.Color.A_SchemeClr.Val := stscvAccent1;
//
//  Result.FAnchor.Objects.Sp.Style.A_EffectRef.Idx := 3;
//  Result.FAnchor.Objects.Sp.Style.A_EffectRef.Color.CreateA_SchemeClr;
//  Result.FAnchor.Objects.Sp.Style.A_EffectRef.Color.A_SchemeClr.Val := stscvAccent1;
//
//  Result.FAnchor.Objects.Sp.Style.A_FontRef.Idx := stfciMinor;
//  Result.FAnchor.Objects.Sp.Style.A_FontRef.Color.CreateA_SchemeClr;
//  Result.FAnchor.Objects.Sp.Style.A_FontRef.Color.A_SchemeClr.Val := stscvAccent1;
//
//  Result.FAnchor.Objects.Sp.SpPr.A_SolidFill.Color.CreateA_SrgbClr;
//  Result.FAnchor.Objects.Sp.SpPr.A_SolidFill.Color.A_SrgbClr.Val := $000000FF;
end;
...
begin
  ...
  Anchor.Objects.Sp.TxBody.Create_PXpgList;
...
begin
  if XlsColorType = xdxctHSL then
    ...
    raise Exception.Create('Color is not HSL');
end;
...
begin
  if XlsColorType = xdxctPreset then
    ...
    raise Exception.Create('Color is not Preset');
end;
...
{$ifdef BABOON}
      Result := $FFFFFF;
{$else}
      case FColorChoice.SysClr.Val of
        stscvScrollBar                : Result := Longword(clScrollBar);
        stscvBackground               : Result := Longword(clBackground);
        stscvActiveCaption            : Result := Longword(clActiveCaption);
        stscvInactiveCaption          : Result := Longword(clInactiveCaption);
        stscvMenu                     : Result := Longword(clMenu);
        stscvWindow                   : Result := Longword(clWindow);
        stscvWindowFrame              : Result := Longword(clWindowFrame);
        stscvMenuText                 : Result := Longword(clMenuText);
        stscvWindowText               : Result := Longword(clWindowText);
        stscvCaptionText              : Result := Longword(clCaptionText);
        stscvActiveBorder             : Result := Longword(clActiveBorder);
        stscvInactiveBorder           : Result := Longword(clInactiveBorder);
        stscvAppWorkspace             : Result := Longword(clAppWorkspace);
        stscvHighlight                : Result := Longword(clHighlight);
        stscvHighlightText            : Result := Longword(clHighlightText);
        stscvBtnFace                  : Result := Longword(clBtnFace);
        stscvBtnShadow                : Result := Longword(clBtnShadow);
        stscvGrayText                 : Result := Longword(clGrayText);
        stscvBtnText                  : Result := Longword(clBtnText);
        stscvInactiveCaptionText      : Result := Longword(clInactiveCaptionText);
        stscvBtnHighlight             : Result := Longword(clBtnHighlight);
        stscv3dDkShadow               : Result := Longword(cl3dDkShadow);
        stscv3dLight                  : Result := Longword(cl3dLight);
        stscvInfoText                 : Result := Longword(clInfoText);
        stscvInfoBk                   : Result := Longword(clInfoBk);
{$ifdef DELPHI_7_OR_LATER}
        stscvHotLight                 : Result := Longword(clHotLight);
        stscvMenuHighlight            : Result := Longword(clMenuHighlight);
        stscvMenuBar                  : Result := Longword(clMenuBar);
        stscvGradientActiveCaption    : Result := Longword(clGradientActiveCaption);
        stscvGradientInactiveCaption  : Result := Longword(clGradientInactiveCaption);
{$endif}
      end;
{$endif}
    end;
    xdxctScheme : begin
      case FColorChoice.SchemeClr.Val of
        ...
        stscvTx1: Result := $00FFFFFF; // ???
        stscvBg2: Result := $00FFFFFF; // ???
        stscvTx2: Result := $00FFFFFF; // ???
...
begin
  if XlsColorType = xdxctScheme then
    ...
    raise Exception.Create('Color is not Scheme');
end;
...
begin
  if XlsColorType = xdxctSystem then
    ...
    raise Exception.Create('Color is not System');
end;
...
begin
  GS := Add;
  GS.StopPosition := 0;
  GS.Color.FColorTransform.AsInteger['tint'] := 66000;
  GS.Color.FColorTransform.AsInteger['satMod'] := 160000;

  GS := Add;
  GS.StopPosition := 0.5;
  GS.Color.FColorTransform.AsInteger['tint'] := 44500;
  GS.Color.FColorTransform.AsInteger['satMod'] := 160000;

  GS := Add;
  GS.StopPosition := 1;
  GS.Color.FColorTransform.AsInteger['tint'] := 23500;
  GS.Color.FColorTransform.AsInteger['satMod'] := 160000;
end;
...
begin
  if FFill is TXLSDrwFillGradient then
    ...
    raise Exception.Create('Fill is not Gradient Fill');
end;
...
begin
  if FFill is TXLSDrwFillNone then
    ...
    raise Exception.Create('Fill is not None Fill');
end;
...
begin
  if FFill is TXLSDrwFillPicture then
    ...
    raise Exception.Create('Fill is not Pictuer Fill');
end;
...
begin
  if FFill is TXLSDrwFillSolid then
    ...
    raise Exception.Create('Fill is not Solid Fill');
end;
...
constructor TXLSDrwLineFillChoice.Create(AFillChoice: TEG_LineFillProperties);
begin
  FFillChoice := AFillChoice;
  if FFillChoice.SolidFill <> Nil then
    SetFillType(xdftSolid)
...
begin
  if FFill is TXLSDrwFillNone then
    ...
    raise Exception.Create('Fill is not No Fill');
end;
...
begin
  if GetFillType <> Value then begin
    ...
    case Value of
      ...
      xdftPicture : raise Exception.Create('Lines can not have picture fill');
    end;
  end;
end;
...
begin
  if CanHaveArrows then begin
    ...
    raise Exception.Create('This line can not have arrows');
end;
...
function TXLSDrawingCharts.Add(AChart: TXPGDocXLSXChart; const AName: AxUCString): TXLSDrawingChart;
begin
  Result := TXLSDrawingChart.Create(FOwner,AChart,AName);
  FItems.Add(Result);
end;
...
function TXLSDrawingCharts.Find(const AName: AxUCString): TXLSDrawingChart;
var
  i: integer;
begin
  for i := 0 to FItems.Count - 1 do begin
...
constructor TXLSDrawingChart.Create(AOwner: TXLSDrawing; AChart: TXPGDocXLSXChart; const AName: AxUCString);
begin
  inherited Create(AOwner);
...
function TXLSDrawingChart.GetChart: TXPGDocXLSXChart;
begin
  Result := FChart;
end;
...
begin
  FChart := AChart.FChart.ChartSpace.Chart;
end;

function TXLSDrawingEditorChart.GetCategories: AxUCString;
begin
  Result := TCT_BarSer(FChart.PlotArea.BarChartXpgList[0].EG_BarChartShared.Series[0]).Cat.NumRef.F;
end;

function TXLSDrawingEditorChart.GetValues: AxUCString;
begin
  Result := TCT_BarSer(FChart.PlotArea.BarChartXpgList[0].EG_BarChartShared.Series[0]).Val.NumRef.F;
end;

procedure TXLSDrawingEditorChart.SetCategories(const Value: AxUCString);
begin
  TCT_BarSer(FChart.PlotArea.BarChartXpgList[0].EG_BarChartShared.Series[0]).Cat.NumRef.F := Value;
end;
...
var
  Ser: TCT_BarSer;
begin
  Ser := TCT_BarSer(FChart.PlotArea.BarChartXpgList[0].EG_BarChartShared.Series[0]);
