...
public
     ...
     procedure MoveItems(const ASrcItem, ADestItem: integer);
...
begin
//  if FIsUpdating then
//    raise Exception.Create('call to SetBlock while IsUpdating');
  FBlock := Value;
  FCurrVectorIndex := -1;
  FVectManager.Vector := Nil;
end;
...
{$ifdef MMU_DEBUG}
  if FBlock.MemSize < 0 then
    raise Exception.Create('Free vector: Memory < 0');
{$endif}
...
var
  ...
  Sz: integer;
begin
  Offs := Longword(APtr) - Longword(FBlock.Memory);
{$ifdef MMU_DEBUG}
  if Offs <> FBlock.MemSize then
    raise Exception.Create('_InsertMem: Offs <> MemSize');
{$endif}
...
{$ifdef MMU_DEBUG}
  if Sz < 0 then
    raise Exception.Create('_InsertMem: Sz < 0');

  if FBlock.MemSize < 0 then
    raise Exception.Create('_InsertMem: Memory < 0');
{$endif}
  FVectManager.Vector := Vector[FCurrVectorIndex];
end;
...
{$ifdef MMU_DEBUG}
    if PXVectHeader(FVector).ItemCount > XVECT_MAX_ITEMS then
      raise Exception.Create('Vector overlow');
{$endif}
  ...
  //#! when inserting new items. Hughe time savings can be done here...
    ...
    try
      ...
      if Rebuild.WrittenMem <> AllocSz then
        raise Exception.Create('Rebuild ex mem error');
    finally
      Rebuild.Free;
    end;
  end;
{$ifdef MMU_DEBUG}
  MemorySize;
  if Result = Nil then
    raise Exception.Create('Result is Nil');
  WalkVector(Nil);
{$endif}
end;
...
{$ifdef MMU_DEBUG}
  if (APtr^ and XVECT_EQCOUNT_EMPTY) <> 0 then
    raise Exception.Create('Empty item excpectd');
{$endif}
...
{$ifdef MMU_DEBUG}
  if (EQ = 0) or (EQ > XVECT_MAX_EQCOUNT) then
    raise Exception.Create('Invalid equal byte');
{$endif}
  XL := Result^;
  Inc(Result);
...
{$ifdef MMU_DEBUG}
    if (XL = 0) or (L_SizeXLate[XL] = 0) then
      raise Exception.Create('Invalid XLate byte');
{$endif}
...
{$ifdef MMU_DEBUG}
  if (n and XVECT_EQCOUNT_EMPTY) <> 0 then
    raise Exception.Create('Last item is empty item');
...
begin
  ...
  PXVectHeader(FVector).ItemCount := AItem + 1;
  PXVectHeader(FVector).OffsetToLastEq := Integer(Result) - Integer(FVector);
...
begin
  ...
  Rebuild := TXLSMMURebuildVector.Create(FVector);
  try
    Rebuild.MoveItems(ASrcItem,ADestItem);
    if Rebuild.ItemCount <= 0 then
      FBlockManager._Delete(MemorySize)
    else begin
      AllocSz := Rebuild.RequiredMem;
...
//  if Cnt <= 0 then
//    raise Exception.Create('Vector is empty');
  ...
  if (AList <> Nil) and (Cnt <> 0) then begin
    raise Exception.Create('Item count error');
  end;
  if (AList <> Nil) and (MemSz <> 0) then begin
    VectError('Memory size error');
  end;
...
procedure TXLSMMURebuildVector.MoveItems(const ASrcItem, ADestItem: integer);
var
  i: integer;
  iMax: integer;
  Delta,D: integer;
begin
  ...
  if Delta > 0 then begin
    ...
    if iMax > High(FItems) then
      iMax := High(FItems);
    for i := iMax downto ADestItem do
      FItems[i] := FItems[i + Delta];
    for i := ASrcItem to ASrcItem + Delta do
      FItems[i] := MAXINT;
  end
  else if Delta < 0 then begin
    D := Abs(Delta);
    for i := ADestItem to FItemCount - D - 1 do
      FItems[i] := FItems[i + D];
  end;
...
{$endif}
  ...
  FWrittenMem := Integer(pVect) - Integer(AVector);
end;
