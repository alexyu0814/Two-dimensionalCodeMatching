...
const XLS_DEFAULT_COLOR_COMMENT = $FFFFE1;
...
const Xc12DefColorScheme: array[TXc12ClrSchemeColor] of longword = (
...
function  Xc12ColorToRGB(const Color: TXc12Color): TXc12RGBColor; overload; {$ifdef D2006PLUS} inline; {$endif}
function  Xc12ColorToRGB(const Color: PXc12Color): TXc12RGBColor; overload; {$ifdef D2006PLUS} inline; {$endif}
// Correct RGB, Red,Green,Blue
function  Xc12ColorToNormalRGB(const Color: TXc12Color): longword; overload;
function  Xc12ColorToNormalRGB(const Color: TXc12Color; const AAutoColor: TXc12RGBColor): longword; overload;
...
function  ShortAreaToRefStr(Col1,Row1,Col2,Row2: integer): AxUCString;
...
function Xc12ColorToRGB(const Color: PXc12Color): longword;
var
  RGB: TRGBRec;
  HLS: THLSRec;
  RRec: TRGBRec;
...
begin
  case Color.ColorType of
    exctAuto   : Result := $000000;
    exctIndexed: Result := Xc12IndexColorPalette[Integer(Color.Indexed)];
    exctRgb    : begin
...
function Xc12ColorToRGB(const Color: TXc12Color): longword;
begin
  Result := Xc12ColorToRGB(@Color);
end;

function  Xc12ColorToNormalRGB(const Color: TXc12Color): longword;
var
  V: longword;
begin
  V := Xc12ColorToRGB(@Color);
  Result := ((V and $000000FF) shl 16) + (V and $0000FF00) + ((V and $00FF0000) shr 16);
end;
...
var
  V: longword;
begin
  if Color.ColorType = exctAuto then
    Result := AAutoColor
  else begin
    V := Xc12ColorToRGB(@Color);
    Result := ((V and $000000FF) shl 16) + (V and $0000FF00) + ((V and $00FF0000) shr 16);
...
begin
  ...
  repeat
    ...
    m := m * 26;
    Dec(j);
  until ((j = 0) or not (AnsiChar(S[j]) in ['A'..'Z']));
...
begin
  ...
  repeat
    ...
    m := m * 26;
    Dec(i);
  until ((i = j) or not (AnsiChar(S[i]) in ['A'..'Z']));
  Dec(Col);
...
begin
  if (Col1 = Col2) and (Row1 = Row2) then
    Result := ColRowToRefStr(Col1,Row1)
  else
    Result := AreaToRefStr(Col1,Row1,Col2,Row2);
end;
...
procedure FillDefaultIndexColors;
var
  i: integer;
begin
