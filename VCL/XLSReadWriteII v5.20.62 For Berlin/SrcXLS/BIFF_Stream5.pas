...
// NOTE! Do not use OleStorage.EnumElements as this will cause a memory leak.
  ...
  FileMode := STGM_READ;
  if OleStorage.OpenStream('Workbook',Nil,STGM_DIRECT or FileMode or STGM_SHARE_EXCLUSIVE,0,OleStream) = S_OK then begin
    Result := xvExcel97;
  end
  else if OleStorage.OpenStream('Book',Nil,STGM_DIRECT or FileMode or STGM_SHARE_EXCLUSIVE,0,OleStream) = S_OK then begin
    Result := xvExcel50;
  end
  else
...
begin
  if FReadCONTINUEActive then
    raise Exception.Create('CONTINUE read active');
...
begin
  if FFileVersion <= xvExcel97 then begin
    ...
    raise Exception.Create('Unhandled Excel version');
end;
