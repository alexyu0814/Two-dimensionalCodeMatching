...
begin
  ...
  i := 2;
  if (Length(FFilename) > 2) and (AnsiChar(FFilename[i]) in [#00,#01,#02]) then
    Inc(i);
  BracketAdded := False;
  Result := '';
...
begin
  if (Length(FName) = 2) and (AnsiChar(FName[2]) in [#0..Char(Ord(bnNone) - 1)]) then
...
begin
  ...
  if not TInternalNames(FParent).FIsUpdating then begin
    if TInternalNames(FParent).FindName(S) >= 0 then
      raise Exception.Create('There is allready an area named ' + S);
  end;
...
begin
  ...
  raise Exception.Create('TODO Get size of expression');
  FormulaSz := 0;
