...
begin
  ...
  while NativeInt(APtgs) < LastPtgs do begin
    case APtgs.Id of
      xptgNone    : raise Exception.CreateFmt('Illegal ptgs %.2X',[APtgs.Id]);
      xptgOpAdd   : begin
        FStack.Op(Spaces + '+');
      ...
      xptgArrayFmlaChild: begin
        ...
        // array formulas.
        if ACells = Nil then
          raise Exception.Create('Decode formula with ACells = Nil');
      ...
      else raise Exception.CreateFmt('Unknown Ptg "%.2X"',[APtgs.Id]);
    end;
    Spaces := '';
...
begin
  Dec(FStackPtr);
  if FStackPtr < 0 then
    raise Exception.Create('Empty decode stack');
...
begin
  ...
  if FStackPtr > High(FStack) then
    raise Exception.Create('Out of decode stack');
