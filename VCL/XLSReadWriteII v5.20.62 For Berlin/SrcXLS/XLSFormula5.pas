...
public
     ...
     procedure AdjustCell(const APtgs: PXLSPtgs; const APtgsSize, ADeltaCol, ADeltaRow: integer);
...
begin
  ...
  if Result then begin
    ...
    if L < 0 then
      raise Exception.CreateFmt('Unknown ptgs %.2X',[FPtgs.Id]);
...
procedure TXLSFormulaHandler.AdjustCell(const APtgs: PXLSPtgs; const APtgsSize, ADeltaCol, ADeltaRow: integer);
var
  C1,R1,C2,R2: integer;
  Mask: integer;
  Ptgs: PXLSPtgs;
    ...
    while Iter.Next do begin
      ...
      case Ptgs.Id of
        xptgRef       : begin
          ...
          if (PXlsPtgsRef(Ptgs).Row and ROW_ABSFLAG) = 0 then begin
        ...
        xptgArea      : begin
          ...
          if (PXlsPtgsArea(Ptgs).Row1 and ROW_ABSFLAG) = 0 then begin
        ...
        xptgRefA97     : begin
          ...
          if (PXlsPtgsRef97(Ptgs).Col and $8000) <> 0 then begin
        ...
        xptgAreaA97    : begin
          ...
          if (PXlsPtgsArea97(Ptgs).Col1 and $8000) <> 0 then begin
    ...
    while Iter.Next do begin
      ...
      case Ptgs.Id of
        ...
        xptgArea1d: begin
          ...
          if not InsideExtent(C1,0,C2,0) then begin
            case Ptgs.Id of
              xptgArea  : Ptgs.Id := xptgAreaErr;
              xptgArea1d: Ptgs.Id := xptgArea1dErr;
            end;
          end;
        end;
    ...
    while Iter.Next do begin
      ...
      case Ptgs.Id of
        ...
        xptgArea1d: begin
          ...
          if not InsideExtent(0,R1,0,R2) then
            Ptgs.Id := xptgAreaErr;
        end;
...
begin
  ...
  for i := 0 to FCalcChain.Count - 1 do begin
    ...
    if Res.RefType = xfrtArea then begin
      ...
      raise Exception.Create('Invalid formula result');
    case Res.ValType of
      xfvtUnknown: ;
      ...
      xfvtAreaList: raise Exception.Create('Invalid formula result');
    end;
  end;
...
begin
  ...
  for i := 0 to FCalcChain.Count - 1 do begin
    ...
    if C.Data = Nil then
      raise Exception.Create('Cell is missing');

    // Exception if C not is formula.
    Cells.FormulaType(@C);
...
//      raise Exception.CreateFmt('Invalid formula result in cell %s',[ColRowToRefStr(Item.Col,Item.Row)]);
    Ok := True;
    case Res.ValType of
...
//  FManager.Errors.ErrorList.Clear;
//  Tokens.Dump(FManager.Errors.ErrorList);
//  FManager.Errors.ErrorList.SaveToFile('d:\temp\temp.txt');
