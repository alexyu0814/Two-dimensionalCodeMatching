...
//    C := (FindWindow('TPropertyInspector', nil) = 0);
    ...
    if A or B or C or D then begin
      ShowMessage('This application was built with a demo version of' + #13 +
...
{$ifdef SHAREWARE}
  for i := 0 to FSheets.Count - 1 do
    FSheets[i].AsString[0,0] := 'XLSReadWriteII Copyright(c) 2015 Axolot Data';
{$endif}
end;
...
begin
  if FVersion in [xvExcel21,xvExcel30] then
    raise Exception.Create('Can not write Excel 2.1/3.0 files');
{$ifdef SHAREWARE}
