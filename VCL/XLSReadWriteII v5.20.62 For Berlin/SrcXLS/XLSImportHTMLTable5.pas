unit XLSImportHTMLTable5;

{-
********************************************************************************
******* XLSReadWriteII V4.00                                             *******
*******                                                                  *******
******* Copyright(C) 1999,2007 Lars Arvidsson, Axolot Data               *******
*******                                                                  *******
...
uses SysUtils, Classes,
     Xc12DataStyleSheet5,
     XLSUtils5, XLSHTMLTypes5, XLSHTMLParse5, XLSReadWriteII5;
...
//* ~param SpecialDecode Default set to False. Don't change this.
//* ~param DecSep Decimal separator character. Default is the Delphi DecimalSeparator global variable value. Change this if numeric values not are imported correct.
//* ~result True if any cells where imported.
function ImportHTMLTable(XLS: TXLSReadWriteII5; SheetIndex,Col,Row: integer; Filename: AxUCString; TableIndex: integer; SpecialDecode: boolean = False; DecSep: WideChar = #0): boolean;
...
begin
  ...
  if FDecSep <> #0 then
    FormatSettings.DecimalSeparator := FDecSep;
  try
    if not FConvertNumValues then
      FXLS[FSheetIndex].AsString[FCurrCol,FCurrRow] := S
    else begin
      try
        FXLS[FSheetIndex].AsFloat[FCurrCol,FCurrRow] := StrToFloat(S);
      except
        FXLS[FSheetIndex].AsString[FCurrCol,FCurrRow] := S;
      end;
    end;
    if (S <> '') and (HRef <> '') then begin
      with FXLS[FSheetIndex].Hyperlinks.Add do begin
        Col1 := FCurrCol;
        Row1 := FCurrRow;
        Col2 := FCurrCol;
        Row2 := FCurrRow;
        Address := HRef;
        FXLS[FSheetIndex].Cell[FCurrCol,FCurrRow].FontColor := $0000FF;
        FXLS[FSheetIndex].Cell[FCurrCol,FCurrRow].FontUnderline := xulSingle;
      end;
    end;
  finally
