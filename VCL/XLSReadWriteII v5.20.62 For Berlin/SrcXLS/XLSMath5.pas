...
begin
   ...
   if z <= 0 then begin
      if Floor(z) = z then
         raise exception.Create('Evaluation  of tgamma at a negative integer');
      if z <= -20 then begin
         result := GammaImp(-z, Lanczos) * sinpx(z);
         if((Abs(result) < 1) and (MaxDouble * Abs(result) < Pi)) then
            raise Exception.Create('Result of tgamma is too large to represent.');
         Result := -Pi / result;
         if result = 0 then
            raise Exception.Create('Result of tgamma is too small to represent.');
         Exit;
      end;
...
begin
   ...
   begin
     raise Exception.Create('only normalized supported');
...
function RisingFactorialRatio(A,B,_P1: double): double;
begin
  raise Exception.Create('TODO RisingFactorialRatio');
end;
...
begin
   ...
   b := bx;
   if (a >= b) then
      raise Exception.Create('Parameters a and b out of order');
   fa := fax;
   fb := fbx;
   ...
   if (Sign(fa) * Sign(fb) > 0) then
         raise Exception.Create('Parameters a and b do not bracket the root.');
     ...
     if (Sign(fb) = Sign(fa)) then begin
        if (fa < 0) then begin
           ...
           while (Sign(fb) = Sign(fa)) do begin
              if (count = 0) then
                 raise Exception.Create('Unable to bracket root');
              a := b;
              fa := fb;
              b := b * multiplier;
              if (b > max_bound) then
...
//   static const char* function = "boost::math::gamma_q_inv<%1%>(%1%, %1%)";
   ...
   if (a <= 0) then
      raise Exception.Create('Argument a in the incomplete gamma function inverse must be >= 0.');
   if ((q < 0) or (q > 1)) then
      raise Exception.Create('Probabilty must be in the range [0,1] in the incomplete gamma function inverse.');
   if (q = 0) then begin
      Result := MaxDouble;
      Exit;
   end;
   ...
   if (guess = lower) then
      raise Exception.Create('Expected result known to be non-zero, but is smaller than the smallest available number.');
   Result := guess;
end;
   ...
   if (x = 0) then begin
     if a > 1 then
       ...
       if a = 1 then
         ...
         raise Exception.Create('IBeta Derivative overlow');
     end;
     Exit;
   end
   else if(x = 1) then begin
...
//   static const char* function = "boost::math::lgamma<%1%>(%1%)";
   ...
   if (z <= 0) then begin
      // reflection formula:
      if (floor(z) = z) then
         raise Exception.Create('Evaluation of lgamma at a negative integer');
...
begin
   if (x = 0) then begin
      // special cases:
      if (df1 < 2) then
        raise Exception.Create('FisherFDistPDF Overflow')
      else if (df1 = 2) then
         Result := 1
      else
         Result := 0;
