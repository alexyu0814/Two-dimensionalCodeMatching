...
begin
  ...
  for i := 1 to Length(S) do begin
    if AnsiChar(S[i]) in [AnsiChar(charQuote1),AnsiChar(charQuote2)] then
      InQuote := not InQuote;
    if not InQuote then
...
begin
  ...
  if FValue[pos] = '$' then
    Inc(pos);
  if (pos = FLen) or not (AnsiChar(FValue[pos]) in [AnsiChar('a')..AnsiChar('z'),AnsiChar('A')..AnsiChar('Z')]) then
    Exit;
  Inc(pos);
  if (AnsiChar(FValue[pos - 1]) in [AnsiChar('a')..AnsiChar('i'),AnsiChar('A')..AnsiChar('I')]) and (AnsiChar(FValue[pos]) in [AnsiChar('a')..AnsiChar('z'),AnsiChar('A')..AnsiChar('Z')]) then
    Inc(pos);
  // maxcol = 255
  if (AnsiChar(FValue[pos - 1]) in [AnsiChar('i'),AnsiChar('I')]) and (AnsiChar(FValue[pos]) in [AnsiChar('w')..AnsiChar('z'),AnsiChar('W')..AnsiChar('Z')]) then
    Exit;
  if FValue[pos] = '$' then
    Inc(pos);
  j := pos;
  while j <= FLen do begin
    if not (AnsiChar(FValue[j]) in [AnsiChar('0')..AnsiChar('9')]) then
      Break;
    Inc(j);
  end;
  if j <= pos then
  ...
  if (j < FLen) and not (AnsiChar(FValue[j]) in [AnsiChar(FormatSettings.ListSeparator),AnsiChar('!'),AnsiChar(' '),AnsiChar('='),AnsiChar(':'),AnsiChar('0')..AnsiChar('9'),AnsiChar('+'),AnsiChar('&'),AnsiChar('-'),AnsiChar('*'),AnsiChar('/'),AnsiChar('%'),AnsiChar('^'),AnsiChar('>'),AnsiChar('<'),AnsiChar('('),AnsiChar(')'),AnsiChar('{'),AnsiChar('}'),AnsiChar('"'),AnsiChar(''''),AnsiChar('['),AnsiChar(']'),AnsiChar('#'),AnsiChar('$')]) then
    Exit;
...
begin
  ...
  while (Result <= FLen) and not (AnsiChar(FValue[Result]) in [AnsiChar(FormatSettings.ListSeparator),AnsiChar('\'),AnsiChar('/'),AnsiChar('?'),AnsiChar('*'),AnsiChar('['),AnsiChar(']'),AnsiChar(' '),AnsiChar('+'),AnsiChar('&'),AnsiChar('-'),AnsiChar('*'),AnsiChar('/'),AnsiChar('%'),AnsiChar('^'),AnsiChar('<'),AnsiChar('>'),AnsiChar('='),AnsiChar('('),AnsiChar(')'),AnsiChar('#'),AnsiChar('{'),AnsiChar('}'),AnsiChar(''''),AnsiChar('"'),AnsiChar('!'),AnsiChar('.')]) do
    Inc(Result);
{
...
begin
  ...
  if ((p - i) >= 7) and (FValue[i + 2] = ':') and (FValue[i + 3] = '\') then begin
    ...
    while (Result <= FLen) and (FValue[Result] <> '''') do begin
      if AnsiChar(FValue[Result]) in [AnsiChar('\'),AnsiChar('/'),AnsiChar('?'),AnsiChar('*'),AnsiChar('['),AnsiChar(']')] then begin
...
begin
  Result := i + 1;
  while (i <= FLen) and (AnsiChar(FValue[Result]) in [AnsiChar('A')..AnsiChar('Z'),AnsiChar('!')]) do
    Inc(Result);
end;
...
// Check this before changing. If above is used, autofilters (see sample) gives error.
      ...
      'A'..'Z': begin
        ...
        if AnsiChar(FValue[i]) = AnsiChar(FormatSettings.ListSeparator) then begin
          Add(FormatSettings.ListSeparator,ttListSep);
          Inc(i);
        end
        else if AnsiChar(FValue[i]) = AnsiChar(FormatSettings.DecimalSeparator) then
