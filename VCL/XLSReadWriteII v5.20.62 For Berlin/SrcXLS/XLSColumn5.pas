...
protected
     ...
     function  DefaultWidth: integer; {$ifdef DELPHI_2006_OR_LATER} inline; {$endif}
...
public
     ...
     // Getter is not equal to setter, PixelWidth2 = PixelWidth2 = False;
...
begin
  Result := XLS_DEFAULT_COLWIDTH / 256;
end;

function TXLSColumn.DefaultWidth: integer;
begin
  Result := XLS_DEFAULT_COLWIDTH;
end;
...
begin
  if (FCol = Nil) and (AValue = DefaultWidth) then
    Exit;
...
begin
  ...
  for i := 1 to Cnt - 1 do begin
    if (AList[i].Min - AList[i - 1].Max) > 1 then
      AList.Add(NewColumn(AList[i - 1].Max + 1,AList[i].Min - 1,FTheColumn.DefaultWidth));
  end;
  if ACol1 < AList[0].Min then
    AList.Add(NewColumn(ACol1,AList[0].Min - 1,FTheColumn.DefaultWidth));
  if ACol2 > AList[Cnt - 1].Max then
    AList.Add(NewColumn(AList[Cnt - 1].Max + 1,ACol2,FTheColumn.DefaultWidth));
end;
...
begin
  ...
  if ACol > 0 then begin
    ...
    if (Xc12Col <> Nil) and ((Xc12Col.Style <> FStyles.XFs.DefaultXF) or (Xc12Col.Width <> (XLS_DEFAULT_COLWIDTH / 256))) then begin
...
begin
  ...
  if (FCols.Count <= 0) or (FCols[FCols.Count - 1].Max < ACol1) then begin
    if AValue = FTheColumn.DefaultWidth then
      Exit;
...
begin
  ...
  if (FCols.Count <= 0) or (FCols[FCols.Count - 1].Max < ACol1) then begin
    ...
    FCols.Add(NewColumn(ACol1,ACol2,XLS_DEFAULT_COLWIDTH),AStyle);
    Result := Nil;
  end
  else begin
