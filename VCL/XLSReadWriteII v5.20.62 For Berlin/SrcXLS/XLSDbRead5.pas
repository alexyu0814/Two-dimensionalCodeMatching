      ...
      if XLSDetails.FIncludeFieldsInx[i] and not XLSDetails.FExcludeFieldsInx[i] then begin
        if not XLSDetails.DataSet.Fields[i].IsNull then begin
          case XLSDetails.DataSet.Fields[i].DataType of
            ...
            ftFixedChar,
            ftGuid:
                        FXLS[FSheet].AsString[ACol,ARow] := XLSDetails.DataSet.Fields[i].AsString;
            ftWideString:
                        FXLS[FSheet].AsString[ACol,ARow] := XLSDetails.DataSet.Fields[i].AsString;
            ftVariant:  FXLS[FSheet].AsVariant[ACol,ARow] := XLSDetails.DataSet.Fields[i].AsVariant;
//            ftMemo,
//            ftFmtMemo:
//                        FXLS[FSheet].AsString[ACol,ARow] := XLSDetails.DataSet.Fields[i].AsString;
...
{$endif}
            ftAutoInc,
            ftFloat:
                        FXLS[FSheet].AsFloat[ACol,ARow] := XLSDetails.DataSet.Fields[i].AsFloat;
            ftBoolean:
                        FXLS[FSheet].AsBoolean[ACol,ARow] := XLSDetails.DataSet.Fields[i].AsBoolean;
            ftGraphic: ;
{$ifndef DELPHI_5}
            ftTimestamp,
{$endif}
            ftDate,
            ftTime,
            ftDateTime: FXLS[FSheet].AsDateTime[ACol,ARow] := XLSDetails.DataSet.Fields[i].AsFloat;
          end;
        end;
        if FFormatCells then begin
          if FXLS[FSheet].Cell[ACol,ARow] = Nil then
            FXLS[FSheet].AsBlank[ACol,ARow] := True;
          if Level <= High(DetailCellColors) then
            FXLS[FSheet].Cell[ACol,ARow].FillPatternForeColor := DetailCellColors[Level]
          else
            FXLS[FSheet].Cell[ACol,ARow].FillPatternForeColor := DetailCellColors[High(DetailCellColors)];
...
begin
  if FIncludeFieldnames then begin
    ...
    for i := 0 to XLSDetails.DataSet.FieldCount - 1 do begin
      if XLSDetails.FIncludeFieldsInx[i] and not XLSDetails.FExcludeFieldsInx[i] then begin
          FXLS[FSheet].AsString[ACol,ARow] := XLSDetails.DataSet.Fields[i].DisplayName;
          if FFormatCells then begin
            FXLS[FSheet].Cell[ACol,ARow].FillPatternForeColor := xcOrange;
            FXLS[FSheet].Cell[ACol,ARow].FontStyle := [xfsBold];
            if Assigned(FDbCellEvent) then
              FDbCellEvent(Self,True,DataSet,i,ACol,ARow);
          end;
        Inc(ACol);
      end;
    end;
