...
begin
  ...
  if xaRead in Cell.F.Assigneds then begin
    ...
    case CellType of
      ...
      stctD  : raise Exception.Create('Date cells not supported');
      stctE  : FCurrSheet.Cells.FormulaHelper.AsErrorStr := Cell.V;
      stctStr: FCurrSheet.Cells.FormulaHelper.AsString := Cell.V;
      else     raise Exception.Create('Unsupported type of formula cell: ' + IntToStr(Integer(Cell.T)));
    end;
...
//          stctStr: raise Exception.Create('Illegal use of formula string');
        end;
      end;
    end;
  end;
...
//        FCurrSheet.Cells.FormulaHelper.Ptgs := Ptgs;
    ...
    if Cell.V = '' then
      ...
      case CellType of
        ...
        stctStr: FManager.EventCell.AsString := Cell.V;
        stctInlineStr: raise Exception.Create('Unhandled string type in file'); // FCurrSheet.IntWriteSSTString(C,R,FI,'Inline string');
      end;
    end;
  end;
      ...
      if N.QName = 'v:shape' then begin
        ...
        if (Col >= 0) and (Row >= 0) then begin
          ...
          if Comment <> Nil then begin
            ...
            if S <> '' then
              Comment.Color := XmlStrHexToIntDef(S,XLS_DEFAULT_COLOR_COMMENT);
...
begin
  ...
  if Stream <> Nil then begin
    ...
    try
      ...
      FCurrSheet.Drawing.LoadFromStream(Stream);
    finally
      Stream.Free;
...
begin
  ...
  try
    ...
    try
      ...
      for i := 0 to List.Count - 1 do begin
        PivotTable := FCurrSheet.PivotTables.Add;

        Strm := FManager.FileData.OPC.ReadPivotTable(List[i]);
        try
          PivotTable.DOM.LoadFromStream(Strm);
        finally
          Strm.Free;
        end;

        if PivotTable.DOM.Root.Count > 0 then begin
          j := PivotTable.DOM.Root[0].Attributes.FindValueIntDef('cacheId',-1);
          PivotTable.Cache := FManager.Workbook.PivotCaches.Find(j);
...
var
  Stream: TStream;
begin
  Stream := FManager.FileData.OPC.ReadVmlDrawing(AId);
  if Stream <> Nil then begin
...
var
  ...
  PivotCache: TXLSPivotCacheDefinition;
  ...
  try
    ...
    try
      ...
      for i := 0 to XML.Workbook.PivotCaches.PivotCacheXpgList.Count - 1 do begin
        PivotCache := FManager.Workbook.PivotCaches.Add;
        ...
        Strm := FManager.FileData.OPC.ReadPivotCacheDefinition(Id);
        try
          PivotCache.DOM.LoadFromStream(Strm);
        finally
          Strm.Free;
        end;

        if PivotCache.DOM.Root.Count > 0 then begin
          S := PivotCache.DOM.Root[0].Attributes.FindValue('r:id');
          if S <> '' then begin
            Strm := FManager.FileData.OPC.ReadPivotCacheRecords(Id,S);
            try
              PivotCache.Records.Data.CopyFrom(Strm,Strm.Size);
            finally
              Strm.Free;
            end;
          end;
