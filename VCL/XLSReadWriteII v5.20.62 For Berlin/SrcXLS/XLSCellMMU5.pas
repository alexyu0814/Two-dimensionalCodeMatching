...
//     function  IterRef: TXLSRefItem; {$ifdef DELPHI_XE_OR_LATER} inline; {$endif}
     ...
     procedure RenewStringCellsIndex(const AIndex: integer);
...
begin
  if (ACell.XLate = 0) or (ACell.Data = Nil) then
    raise Exception.Create('Invalid formula');
...
begin
  case FFormulaHelper.CellType of
    ...
    else               raise Exception.Create('Invalid formula cell type');
  end;
...
begin
  case AXLate and XVECT_XLATE_MASK of
    ...
    else                 raise Exception.Create('Invalid XLate code');
  end;
end;
...
{$endif}
    ...
    case XLate and XVECT_XLATE_MASK of
      ...
      else raise Exception.Create('Invalid XLate');
    end;
...
begin
  if (ACell.Data <> Nil) and ((ACell.XLate and XVECT_XLATE_EX) <> 0) then begin
    ...
    raise Exception.Create('Cell is not formula');
end;
...
procedure TXLSCellMMU.CalcDimensions;
var
  i: integer;
  C1,C2: integer;
  R1,R2: integer;
...
begin
  ...
  if PByte(Result)^ = CELLMMU_FMLA_IS_PTGS then
    ...
    raise Exception.Create('Formula is not compiled');
  end;
end;
...
begin
  ...
  case PByte(P)^ of
    CELLMMU_FMLA_IS_PTGS  : raise Exception.Create('Formula is compiled');
...
begin
  case AXLate and XVECT_XLATE_MASK of
    ...
    else raise Exception.CreateFmt('Invalid XLate value "%d"',[AXLate]);
  end;
  if (AXLate and CELLXL_FLAG_STYLE) <> 0 then
...
begin
  ...
  case FIterate.Cell.XLate and XVECT_XLATE_MASK of
    ...
    CELLXL_NUM_SINGLE  : Result := PSingle(P)^;
    CELLXL_NUM_DOUBLE  : Result := PDouble(P)^;
    else                  raise Exception.Create('Invalid float XLate');
  end;
end;
...
begin
  ...
  case FIterate.Cell.XLate and XVECT_XLATE_MASK of
    ...
    else            raise Exception.Create('Invalid string XLate');
  end;
end;
...
procedure TXLSCellMMU.RenewStringCellsIndex(const AIndex: integer);
begin
  ...
  case FFormulaHelper.FormulaType of
    ...
    xcftArray         : begin
      case PByte(P)^ of
        ...
        else raise Exception.Create('Invalid stored formula');
      end;
    end;
    xcftDataTable: begin
      Inc(P,SizeOf(byte));
...
//      FOwner.StoreFloat(Col,Row,Format,ValFloat);
      ...
      raise Exception.Create('Invalid debug data: ' + S2);
  end;
end;
...
begin
  ...
  if TXc12CellError(FResult) = errUnknown then
    raise Exception.Create('Invalid error value');
