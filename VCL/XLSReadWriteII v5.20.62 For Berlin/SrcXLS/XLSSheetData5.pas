...
{$endif}
     ...
     Xc12DataStyleSheet5,XLSValidate5, XLSMergedCells5, XLSHyperlinks5, XLSCondFormat5,
     XLSAutofilter5, XLSDrawing5, XLSDecodeFmla5, XLSTextUtils5, XLSRTFReadWrite5,
     XLSFormula5, XLSMask5, XLSFormulaTypes5, XLSNames5, XLSClassFactory5, XLSComment5,
{$ifdef _AXOLOT_BETA}
     XLSChart5,
{$endif}
     XLSFormattedObj5, XLSRange5, XLSCmdFormat5,
{$ifdef BABOON}
{$else}
     XLSClipboard5,
{$endif}
...
protected
     ...
     function  DrawingAddImage97(AAnchor: TCT_TwoCellAnchor; const AColOffs,ARowOffs: double): boolean;
public
...
{$endif}
     ...
     procedure Assign(Source: TXLSWorksheet);
     ...
     function  AutoWidthCol(const ACol: integer): integer;
     ...
     function  AutoHeightRows(const Row1,Row2: integer): boolean;
     ...
     procedure CopyCell(const ACol, ARow, ADestCol, ADestRow: integer);
     ...
     procedure CelarCells(const ACol1, ARow1, ACol2, ARow2: integer);

     procedure MoveCell(const ACol, ARow, ADestCol, ADestRow: integer);
...
{$endif}
     ...
     function  AddImage97(AAnchor: TCT_TwoCellAnchor; const ASheetIndex: integer; const AColOffs,ARowOffs: double): boolean;  virtual;
...
{$endif}
     ...
     procedure DeleteCells(const ASheet: integer; AAreas: TBaseCellAreas); overload;
     procedure DeleteCells(const ASheet: integer; AArea: TCellArea); overload;
     procedure DeleteCells(const ASheet, ACol1, ARow1, ACol2, ARow2: integer); overload;
...
procedure TXLSWorksheet.Assign(Source: TXLSWorksheet);
begin
  raise Exception.Create('Not implemented');
end;

function TXLSWorksheet.AutoHeightRow(const ARow: integer): boolean;
{$ifdef BABOON}
begin
  Result := False;
end;
...
function TXLSWorksheet.AutoHeightRows(const Row1, Row2: integer): boolean;
var
  i: integer;
begin
  Result := False;
  for i := Row1 to Row2 do begin
    if AutoHeightRow(i) then
      Result := True;
  end;
end;
  ...
  try
    ...
    while FCells.IterateNextRow do begin
      if FCells.FindCell(ACol,FCells.IterCellRow,C) and (FMergedCells.CellInAreas(ACol,FCells.IterCellRow) <= -1) then begin
        ...
        // Vertical
        if XF.Alignment.Rotation in [90,180,255] then
          W := Canvas.TextHeight(self.GetAsFmtString(ACol,FCells.IterCellRow)) + TM.tmAveCharWidth + 5
        else
...
procedure TXLSWorksheet.AutoWidthCols(const Col1, Col2: integer);
var
  i: integer;
begin
...
procedure TXLSWorksheet.CelarCells(const ACol1, ARow1, ACol2, ARow2: integer);
begin
...
procedure TXLSWorksheet.CopyCell(const ACol, ARow, ADestCol, ADestRow: integer);
var
  Ptgs: PXLSPtgs;
  Sz: integer;
    ...
    if Cell.Data <> Nil then begin
      try
        ...
        if FCells.CellType(@NewCell) in XLSCellTypeFormulas then begin
          ...
          FOwner.FFormulas.AdjustCell(Ptgs,Sz,ADestCol - ACol,ADestRow - ARow);
        end;
      finally
    ...
    if ColsBackward and RowsBackward then begin
      for R := ARow2 downto ARow1 do begin
        for C := ACol2 downto ACol1 do
          CopyCell(C,R,ADestCol + (C - ACol1),ADestRow + (R - ARow1));
      end;
    end
...
begin
  ...
  if xdoAutofilters in AOptions then
    FAutofilter.Delete(ACol, ARow, ACol, ARow);
...
begin
  ...
  if xdoAutofilters in AOptions then
    FAutofilter.Delete(ACol1, ARow1, ACol2, ARow2);
...
begin
  ...
  MoveCellObjects(ACol2 + 1,0,XLS_MAXCOL,XLS_MAXROW,ACol1 - ACol2 - 1,0);
//  MoveCellObjects(ACol1,0,ACol2,XLS_MAXROW,ACol1 - ACol2 - 1,0);
...
begin
  ...
  MoveCellObjects(0, ARow2 + 1, XLS_MAXCOL, XLS_MAXROW,0,ARow1 - ARow2 - 1);
...
function TXLSWorksheet.DrawingAddImage97(AAnchor: TCT_TwoCellAnchor; const AColOffs,ARowOffs: double): boolean;
begin
  Result := FOwner.AddImage97(AAnchor,Index,AColOffs,ARowOffs);
end;
...
begin
  if FCells.FindCell(ACol,ARow,Cell) then begin
    case FCells.CellType(@Cell) of
      ...
      else               raise Exception.Create('Cell is missing/is of wrong type');
    end;
  end
  else
...
//  if Result = Nil then
//    raise Exception.Create('No cell at ' + ColRowToRefStr(ACol,ARow));
end;
...
begin
  FCells.BeginIterate;
  Result := not FCells.IterateNextCol;
end;
...
begin
  ...
  try
    ...
    if (FCells.CellType(@ACell) = xctString) and FManager.SST.IsFormatted[i] then begin
      ...
      Result.AssignText(FManager.SST.GetText(XStr),FManager.StyleSheet.XFs[j].Font,Runs,FManager.SST.GetFontRunsCount(XStr));
    end
    else
      Result.AssignText(GetFormatString(ACell,Nil),FManager.StyleSheet.XFs[j].Font);
  except
    Result.Free;
    Result := Nil;
  end;
...
procedure TXLSWorksheet.MoveCell(const ACol, ARow, ADestCol, ADestRow: integer);
begin
  if InsideExtent(ADestCol,ADestRow) then begin
...
begin
  if ccoCopyValues in ACopyOptions then begin
    ...
    if ColsBackward and RowsBackward then begin
      for R := ARow2 downto ARow1 do begin
        for C := ACol2 downto ACol1 do
          MoveCell(C,R,ADestCol + (C - ACol1),ADestRow + (R - ARow1));
      end;
    end
    else if ColsBackward and not RowsBackward then begin
      for R := ARow1 to ARow2 do begin
...
begin
  ...
  try
    ...
    SetLength(FR,RTF.FontRuns.Count);
    for i := 0 to RTF.FontRuns.Count - 1 do begin
      FR[i].Index := RTF.FontRuns[i].Index;
      if FR[i].Font = Nil then begin
        FR[i].Font := FManager.StyleSheet.Fonts.Add;
        FR[i].Font.Assign(RTF.FontRuns[i]);
      end
      else
        FR[i].Font := FManager.StyleSheet.Fonts.Find(FR[i].Font);
      FManager.StyleSheet.XFEditor.UseFont(FR[i].Font);
    end;

    RichTextCell(ACol,ARow,RTF.Text,FR);
...
begin
  if FOwner.FDefaultFormat <> Nil then
    FCells.StoreBlank(ACol,ARow,FOwner.FDefaultFormat.XF.Index)
  else
    FCells.StoreBlank(ACol,ARow,GetDefaultFormat(ACol,ARow));
end;
...
begin
  if FOwner.FDefaultFormat <> Nil then
    FCells.UpdateBoolean(ACol,ARow,Value,FOwner.FDefaultFormat.XF.Index)
  else
    FCells.UpdateBoolean(ACol,ARow,Value,GetDefaultFormat(ACol,ARow));
end;
...
begin
  if FOwner.FDefaultFormat <> Nil then
    FCells.UpdateFloat(ACol,ARow,Value,FOwner.FDefaultFormat.XF.Index)
  else begin
    ...
    FCells.UpdateFloat(ACol,ARow,Value,XF.Index);
  end;
end;
...
begin
  if FOwner.FDefaultFormat <> Nil then
    FCells.UpdateError(ACol,ARow,Value,FOwner.FDefaultFormat.XF.Index)
  else
    FCells.UpdateError(ACol,ARow,Value,GetDefaultFormat(ACol,ARow));
end;
...
begin
  if FOwner.FDefaultFormat <> Nil then
    FCells.UpdateFloat(ACol,ARow,AValue,FOwner.FDefaultFormat.XF.Index)
  else
    FCells.UpdateFloat(ACol,ARow,AValue,GetDefaultFormat(ACol,ARow));
end;
...
{$endif}
    Sz := FOwner.FFormulas.EncodeFormula(Value,Ptgs,FIndex,ACol,ARow,False);
    if Sz > 0 then begin
...
begin
  if FOwner.FDefaultFormat <> Nil then
    ...
    FCells.UpdateFloat(ACol,ARow,Value,GetDefaultFormat(ACol,ARow));
end;
...
begin
  if FOwner.FDefaultFormat <> Nil then
    FCells.UpdateString(ACol,ARow,Value,FOwner.FDefaultFormat.XF.Index)
  else
    FCells.UpdateString(ACol,ARow,Value,GetDefaultFormat(ACol,ARow));
end;
...
{$else}
    raise Exception.Create('AsVariant not supported by Delphi 5');
{$endif}
  end
  else begin
...
procedure TXLSWorksheet.__TEST;
begin
  FCells.InsertRows(2,1);
end;
...
function TXLSWorkbook.AddImage97(AAnchor: TCT_TwoCellAnchor; const ASheetIndex: integer; const AColOffs,ARowOffs: double): boolean;
begin
  Result := False;
end;
...
begin
  if ASrcSheet = ADestSheet then
    ...
    CopyCells(ASrcSheet,ACol1,R1,ACol2,R2,ADestSheet,ADestCol,0);
...
begin
  if ASrcSheet = ADestSheet then
    ...
    CopyCells(ASrcSheet,C1,ARow1,C2,ARow2,ADestSheet,0,ADestRow);
...
begin
  if ASrcSheet <> ADestSheet then begin
    ...
    CopyCells(ASrcSheet,C1,R1,C2,R2,ADestSheet,C1,R1);
...
procedure TXLSWorkbook.DeleteCells(const ASheet, ACol1, ARow1, ACol2, ARow2: integer);
begin
  Items[ASheet].DeleteCells(ACol1,ARow1,ACol2,ARow2);
end;

procedure TXLSWorkbook.DeleteCells(const ASheet: integer; AAreas: TBaseCellAreas);
var
  i: integer;
begin
  for i := 0 to AAreas.Count - 1 do
    DeleteCells(ASheet,AAreas[i]);
end;

procedure TXLSWorkbook.DeleteCells(const ASheet: integer; AArea: TCellArea);
begin
  Items[ASheet].DeleteCells(AArea.Col1,AArea.Row1,AArea.Col2,AArea.Row2);
end;
...
begin
  ...
  raise Exception.Create('Can not create unique worksheet name');
end;
