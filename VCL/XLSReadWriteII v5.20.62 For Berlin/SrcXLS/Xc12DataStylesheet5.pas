...
begin
  ...
  try
    ...
    if not ((FFill.FgColor.ColorType = exctAuto) or ((FFill.FgColor.ColorType = exctIndexed) and (FFill.FgColor.Indexed = xcAutomatic))) then
      Result := Result + 'background:' + Format(' #%.6X;',[Xc12ColorToNormalRGB(FFill.FgColor)]);
...
begin
  if ABorder.Style = cbsNone then
    ...
    if ABorder.Color.ColorType = exctAuto then
      ...
      Result := Result + Format(' #%.6X;',[Xc12ColorToNormalRGB(ABorder.Color)]);
  end;
end;
...
begin
  ...
  try
    ...
    Result := Result + Format('color:#%.6x;',[Xc12ColorToNormalRGB(FColor)]);
...
{$ifdef XLS_XSS}
  ...
  FColor.ColorType := exctRgb;
  FColor.ARGB := AItem.Color;
...
procedure TXc12Font.Excel97Use;
begin
  IncRef;
end;
...
begin
  Result := (foWrapText in FOptions) or (FHorizAlignment in [chaJustify,chaDistributed]);
end;
...
begin
  if FAlignment <> Nil then
    raise Exception.Create('DXF->Alignment is allready set');
  FAlignment := TXc12CellAlignment.Create;
  Result := FAlignment;
end;
...
begin
  if FBorder <> Nil then
    raise Exception.Create('DXF->Border is allready set');
...
begin
  if FFill <> Nil then
    raise Exception.Create('DXF->Fill is allready set');
...
begin
  if FFont <> Nil then
    raise Exception.Create('DXF->Font is allready set');
...
begin
  if FNumFmt <> Nil then
    raise Exception.Create('DXF->NumFmt is allready set');
...
{$ifdef _AXOLOT_DEBUG}
  if (XF.NumFmt.RefCount < 0) or (XF.Font.RefCount < 0) or (XF.Fill.RefCount < 0) or (XF.Border.RefCount < 0) or (XF.RefCount < 0) then
    raise Exception.Create('Ref count < 0');
{$endif}
end;
...
begin
  ...
  if Result <> Nil then
    ...
    raise Exception.Create('Can not find font');
end;
