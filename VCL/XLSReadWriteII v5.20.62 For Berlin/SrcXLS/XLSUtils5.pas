...
const CurrentVersionNumber = '5.20.62';
// Max 4 digits
const CurrentBuildNumber = '5259';

// C++ Builder don't like the name MININT as it's already defined.
const XLS_MININT    = -(MAXINT - 1);
const MINDOUBLE     = -1.79769313486232e308;
const MAXDOUBLE     = 1.79769313486232e308;
...
function  StreamIsZIP(const AStream: TStream): boolean;
...
function  PosEx(const FindText,WithinText: AxUCString; const StartIndex: integer): integer;
begin
  raise Exception.Create('TODO D6');
end;
...
begin
  AStream.Read(Magic,Length(Magic));
  AStream.Seek(-Length(Magic),soFromCurrent);
  Result := (Magic[0] = $50) and (Magic[1] = $4B) and (Magic[2] = $03) and (Magic[3] = $04);
end;
...
begin
  case AErrorNo of
    ...
    XLSERR_DIRECTWR_CELL_NOT_INC       : Result := 'Direct write: Cell columna and row can not be the same as previous';

    3001                               : Result := Format('Can not find Defined Name "%s"',[AText]);
    XLSWARN_CIRCULAR_FORMULA           : Result := Format('Circular formula "%s"',[AText]);
    XLSWARN_USEDSHEETNAME              : Result := Format('There is allready a sheet with name "%s"',[AText]);
    ...
    XLSERR_FMLA_MISSINGRPAR            : Result := 'Missing right paranthesis';
    ...
    XLSERR_FMLA_UNKNOWNSHEET           : Result := Format('Uknown sheet name "%s"',[AText]);
    XLSERR_FMLA_UNKNOWNXBOOK           : Result := Format('Uknown external sheet "%s"',[AText]);
    XLSERR_FMLA_BADUSEOFOP             : Result := 'Invalid use of operator';
    XLSERR_FMLA_UNKNOWNNAME            : Result := Format('Uknown name "%s"',[AText]);
    ...
    XLSERR_FMLA_BADUSEOFTBLSPEC        : Result := 'Invalid combination if table specials';
    XLSERR_FMLA_TBLSPECNOTFIRST        : Result := 'Table specials must be first in expression';

    XLSERR_NAME_EMPTY                  : Result := 'Name text is empty';
    XLSERR_NAME_DUPLICATE              : Result := Format('Name "%s" is allready defined',[AText]);
