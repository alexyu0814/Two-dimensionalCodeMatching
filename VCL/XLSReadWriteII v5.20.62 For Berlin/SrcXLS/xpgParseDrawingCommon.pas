...
protected
     ...
     procedure ReadAddValue(const AName,AValue: AxUCString);
     procedure GetNameValue(const AIndex: integer; out AName,AValue: AxUCString);
...
public
     ...
     TEG_LineFillProperties = class(TXPGBase)
protected
     FA_NoFill: TCT_NoFillProperties;
     FA_SolidFill: TCT_SolidColorFillProperties;
     FA_GradFill: TCT_GradientFillProperties;
...
public
     ...
     destructor Destroy; override;
     procedure Clear;
     procedure Assign(AItem: TEG_LineFillProperties);
     procedure CopyTo(AItem: TEG_LineFillProperties);
     function  Create_NoFill: TCT_NoFillProperties;
     function  Create_SolidFill: TCT_SolidColorFillProperties;
     function  Create_GradFill: TCT_GradientFillProperties;
     function  Create_PattFill: TCT_PatternFillProperties;
...
protected
     ...
     FA_EG_LineFillProperties: TEG_LineFillProperties;
...
public
     ...
     property LineFillProperties: TEG_LineFillProperties read FA_EG_LineFillProperties;
...
public
     ...
     function  Create_PXpgList: TCT_TextParagraphXpgList;
...
begin
  ...
  if i >= 0 then
    ...
    raise Exception.CreateFmt('Can not find enum "%s"',[AValue]);
end;
...
var
  i: integer;
  Name,Value: AxUCString;
begin
  for i := 0 to FItems.Count - 1 do begin
    GetNameValue(i,Name,Value);
    AWriter.AddAttribute('val',Value);
    AWriter.SimpleTag('a:' + Name);
  end;
end;
...
begin
  for Result := 0 to FItems.Count - 1 do begin
    GetNameValue(Result,Name,Value);
    if Name = AName then
      Exit;
  end;
  Result := -1;
...
procedure TEG_ColorTransform.GetNameValue(const AIndex: integer; out AName, AValue: AxUCString);
var
  S: AxUCString;
  p: integer;
begin
...
begin
  ...
  Value := AReader.Attributes.Values[0];
...
procedure TEG_ColorTransform.ReadAddValue(const AName,AValue: AxUCString);
begin
...
begin
  i := Find(AName);
  if i >= 0 then
    FItems[i] := AName + '=' + IntToStr(Round(Value))
  else
    FItems.Add(AName + '=' + IntToStr(Round(Value)));
end;
...
begin
  i := Find(AName);
  if i >= 0 then
    FItems[i] := AName + '=' + IntToStr(Value)
  else
    FItems.Add(AName + '=' + IntToStr(Value));
end;
...
begin
  Result := TCT_RelativeRect.Create(FOwner);
  FA_FillRect := Result;
end;
...
begin
  Result := TCT_GradientStopXpgList.Create(FOwner);
  FA_GsXpgList := Result;
end;
...
begin
  Result := TCT_LinearShadeProperties.Create(FOwner);
  FA_Lin := Result;
end;

function  TEG_ShadeProperties.Create_Path: TCT_PathShadeProperties;
begin
  Result := TCT_PathShadeProperties.Create(FOwner);
  FA_Path := Result;
end;
...
begin
  ...
  if FA_NoFill <> Nil then 
    Inc(ElemsAssigned,FA_NoFill.CheckAssigned);
...
begin
  Result := TCT_EffectContainerXpgList.Create(FOwner);
  FA_ContXpgList := Result;
end;

function  TEG_Effect.Create_A_EffectXpgList: TCT_EffectReferenceXpgList;
begin
  Result := TCT_EffectReferenceXpgList.Create(FOwner);
  FA_EffectXpgList := Result;
end;

function  TEG_Effect.Create_A_AlphaBiLevelXpgList: TCT_AlphaBiLevelEffectXpgList;
begin
  Result := TCT_AlphaBiLevelEffectXpgList.Create(FOwner);
  FA_AlphaBiLevelXpgList := Result;
end;

function  TEG_Effect.Create_A_AlphaCeilingXpgList: TCT_AlphaCeilingEffectXpgList;
begin
  Result := TCT_AlphaCeilingEffectXpgList.Create(FOwner);
  FA_AlphaCeilingXpgList := Result;
end;

function  TEG_Effect.Create_A_AlphaFloorXpgList: TCT_AlphaFloorEffectXpgList;
begin
  Result := TCT_AlphaFloorEffectXpgList.Create(FOwner);
  FA_AlphaFloorXpgList := Result;
end;

function  TEG_Effect.Create_A_AlphaInvXpgList: TCT_AlphaInverseEffectXpgList;
begin
  Result := TCT_AlphaInverseEffectXpgList.Create(FOwner);
  FA_AlphaInvXpgList := Result;
end;

function  TEG_Effect.Create_A_AlphaModXpgList: TCT_AlphaModulateEffectXpgList;
begin
  Result := TCT_AlphaModulateEffectXpgList.Create(FOwner);
  FA_AlphaModXpgList := Result;
end;

function  TEG_Effect.Create_A_AlphaModFixXpgList: TCT_AlphaModulateFixedEffectXpgList;
begin
  Result := TCT_AlphaModulateFixedEffectXpgList.Create(FOwner);
  FA_AlphaModFixXpgList := Result;
end;

function  TEG_Effect.Create_A_AlphaOutsetXpgList: TCT_AlphaOutsetEffectXpgList;
begin
  Result := TCT_AlphaOutsetEffectXpgList.Create(FOwner);
  FA_AlphaOutsetXpgList := Result;
end;

function  TEG_Effect.Create_A_AlphaReplXpgList: TCT_AlphaReplaceEffectXpgList;
begin
  Result := TCT_AlphaReplaceEffectXpgList.Create(FOwner);
  FA_AlphaReplXpgList := Result;
end;

function  TEG_Effect.Create_A_BiLevelXpgList: TCT_BiLevelEffectXpgList;
begin
  Result := TCT_BiLevelEffectXpgList.Create(FOwner);
  FA_BiLevelXpgList := Result;
end;

function  TEG_Effect.Create_A_BlendXpgList: TCT_BlendEffectXpgList;
begin
  Result := TCT_BlendEffectXpgList.Create(FOwner);
  FA_BlendXpgList := Result;
end;

function  TEG_Effect.Create_A_BlurXpgList: TCT_BlurEffectXpgList;
begin
  Result := TCT_BlurEffectXpgList.Create(FOwner);
  FA_BlurXpgList := Result;
end;

function  TEG_Effect.Create_A_ClrChangeXpgList: TCT_ColorChangeEffectXpgList;
begin
  Result := TCT_ColorChangeEffectXpgList.Create(FOwner);
  FA_ClrChangeXpgList := Result;
end;

function  TEG_Effect.Create_A_ClrReplXpgList: TCT_ColorReplaceEffectXpgList;
begin
  Result := TCT_ColorReplaceEffectXpgList.Create(FOwner);
  FA_ClrReplXpgList := Result;
end;

function  TEG_Effect.Create_A_DuotoneXpgList: TCT_DuotoneEffectXpgList;
begin
  Result := TCT_DuotoneEffectXpgList.Create(FOwner);
  FA_DuotoneXpgList := Result;
end;

function  TEG_Effect.Create_A_FillXpgList: TCT_FillEffectXpgList;
begin
  Result := TCT_FillEffectXpgList.Create(FOwner);
  FA_FillXpgList := Result;
end;

function  TEG_Effect.Create_A_FillOverlayXpgList: TCT_FillOverlayEffectXpgList;
begin
  Result := TCT_FillOverlayEffectXpgList.Create(FOwner);
  FA_FillOverlayXpgList := Result;
end;

function  TEG_Effect.Create_A_GlowXpgList: TCT_GlowEffectXpgList;
begin
  Result := TCT_GlowEffectXpgList.Create(FOwner);
  FA_GlowXpgList := Result;
end;

function  TEG_Effect.Create_A_GraysclXpgList: TCT_GrayscaleEffectXpgList;
begin
  Result := TCT_GrayscaleEffectXpgList.Create(FOwner);
  FA_GraysclXpgList := Result;
end;

function  TEG_Effect.Create_A_HslXpgList: TCT_HSLEffectXpgList;
begin
  Result := TCT_HSLEffectXpgList.Create(FOwner);
  FA_HslXpgList := Result;
end;

function  TEG_Effect.Create_A_InnerShdwXpgList: TCT_InnerShadowEffectXpgList;
begin
  Result := TCT_InnerShadowEffectXpgList.Create(FOwner);
  FA_InnerShdwXpgList := Result;
end;

function  TEG_Effect.Create_A_LumXpgList: TCT_LuminanceEffectXpgList;
begin
  Result := TCT_LuminanceEffectXpgList.Create(FOwner);
  FA_LumXpgList := Result;
end;

function  TEG_Effect.Create_A_OuterShdwXpgList: TCT_OuterShadowEffectXpgList;
begin
  Result := TCT_OuterShadowEffectXpgList.Create(FOwner);
  FA_OuterShdwXpgList := Result;
end;

function  TEG_Effect.Create_A_PrstShdwXpgList: TCT_PresetShadowEffectXpgList;
begin
  Result := TCT_PresetShadowEffectXpgList.Create(FOwner);
  FA_PrstShdwXpgList := Result;
end;

function  TEG_Effect.Create_A_ReflectionXpgList: TCT_ReflectionEffectXpgList;
begin
  Result := TCT_ReflectionEffectXpgList.Create(FOwner);
  FA_ReflectionXpgList := Result;
end;

function  TEG_Effect.Create_A_RelOffXpgList: TCT_RelativeOffsetEffectXpgList;
begin
  Result := TCT_RelativeOffsetEffectXpgList.Create(FOwner);
  FA_RelOffXpgList := Result;
end;

function  TEG_Effect.Create_A_SoftEdgeXpgList: TCT_SoftEdgesEffectXpgList;
begin
  Result := TCT_SoftEdgesEffectXpgList.Create(FOwner);
  FA_SoftEdgeXpgList := Result;
end;

function  TEG_Effect.Create_A_TintXpgList: TCT_TintEffectXpgList;
begin
  Result := TCT_TintEffectXpgList.Create(FOwner);
  FA_TintXpgList := Result;
end;

function  TEG_Effect.Create_A_XfrmXpgList: TCT_TransformEffectXpgList;
begin
  Result := TCT_TransformEffectXpgList.Create(FOwner);
  FA_XfrmXpgList := Result;
end;
...
begin
  Result := TCT_DashStopXpgList.Create(FOwner);
  FA_DsXpgList := Result;
end;
...
{ TEG_LineFillProperties }

function  TEG_LineFillProperties.CheckAssigned: integer;
var
  ElemsAssigned: integer;
begin
  ElemsAssigned := 0;
...
function  TEG_LineFillProperties.HandleElement(AReader: TXpgReadXML): TXPGBase;
begin
  case AReader.QNameHashA of
    $000002FF: begin
      if FA_NoFill = Nil then 
...
procedure TEG_LineFillProperties.Write(AWriter: TXpgWriteXML);
begin
  if FA_NoFill <> Nil then
    AWriter.SimpleTag('a:noFill');
  if (FA_SolidFill <> Nil) and FA_SolidFill.Assigned then
...
constructor TEG_LineFillProperties.Create(AOwner: TXPGDocBase);
begin
  FOwner := AOwner;
  FElementCount := 4;
  FAttributeCount := 0;
...
destructor TEG_LineFillProperties.Destroy;
begin
  if FA_NoFill <> Nil then 
    FA_NoFill.Free;
  if FA_SolidFill <> Nil then 
...
procedure TEG_LineFillProperties.Clear;
begin
  FAssigneds := [];
  if FA_NoFill <> Nil then 
    FreeAndNil(FA_NoFill);
...
procedure TEG_LineFillProperties.Assign(AItem: TEG_LineFillProperties);
begin
end;

procedure TEG_LineFillProperties.CopyTo(AItem: TEG_LineFillProperties);
begin
end;

function  TEG_LineFillProperties.Create_NoFill: TCT_NoFillProperties;
begin
  if FA_NoFill = Nil then begin
    Clear;
    FA_NoFill := TCT_NoFillProperties.Create(FOwner);
...
function  TEG_LineFillProperties.Create_SolidFill: TCT_SolidColorFillProperties;
begin
  if FA_SolidFill = Nil then begin
    Clear;
    FA_SolidFill := TCT_SolidColorFillProperties.Create(FOwner);
...
function  TEG_LineFillProperties.Create_GradFill: TCT_GradientFillProperties;
begin
  if FA_GradFill = Nil then begin
    Clear;
    FA_GradFill := TCT_GradientFillProperties.Create(FOwner);
...
function  TEG_LineFillProperties.Create_PattFill: TCT_PatternFillProperties;
begin
  if FA_PattFill = Nil then begin
    Clear;
    FA_PattFill := TCT_PatternFillProperties.Create(FOwner);
...
begin
  if (FA_Round <> Nil) and FA_Round.Assigned then 
    AWriter.SimpleTag('a:round');
  if (FA_Bevel <> Nil) and FA_Bevel.Assigned then 
    AWriter.SimpleTag('a:bevel');
  if (FA_Miter <> Nil) and FA_Miter.Assigned then 
  begin
...
begin
  ...
  if (FA_HeadEnd <> Nil) and FA_HeadEnd.Assigned then 
  begin
    FA_HeadEnd.WriteAttributes(AWriter);
    AWriter.SimpleTag('a:headEnd');
  end;
  if (FA_TailEnd <> Nil) and FA_TailEnd.Assigned then 
  begin
...
begin
  ...
  FElementCount := 6;
  FAttributeCount := 4;
  FA_EG_LineFillProperties := TEG_LineFillProperties.Create(FOwner);
...
begin
  Result := TCT_EffectList.Create(FOwner);
  FA_EffectLst := Result;
end;
...
begin
  Result := TCT_TextUnderlineLineFollowText.Create(FOwner);
  FA_ULnTx := Result;
end;
...
begin
  Result := TCT_TextUnderlineFillFollowText.Create(FOwner);
  FA_UFillTx := Result;
end;

function  TEG_TextUnderlineFill.Create_A_UFill: TCT_TextUnderlineFillGroupWrapper;
begin
  Result := TCT_TextUnderlineFillGroupWrapper.Create(FOwner);
  FA_UFill := Result;
end;
...
begin
  Result := TCT_EmbeddedWAVAudioFile.Create(FOwner);
  FA_Snd := Result;
end;
...
begin
  Result := TCT_TextSpacingPercent.Create(FOwner);
  FA_SpcPct := Result;
end;

function  TCT_TextSpacing.Create_A_SpcPts: TCT_TextSpacingPoint;
begin
  Result := TCT_TextSpacingPoint.Create(FOwner);
  FA_SpcPts := Result;
end;
...
begin
  Result := TCT_TextBulletColorFollowText.Create(FOwner);
  FA_BuClrTx := Result;
end;
...
begin
  Result := TCT_TextBulletSizeFollowText.Create(FOwner);
  FA_BuSzTx := Result;
end;

function  TEG_TextBulletSize.Create_A_BuSzPct: TCT_TextBulletSizePercent;
begin
  Result := TCT_TextBulletSizePercent.Create(FOwner);
  FA_BuSzPct := Result;
end;

function  TEG_TextBulletSize.Create_A_BuSzPts: TCT_TextBulletSizePoint;
begin
  Result := TCT_TextBulletSizePoint.Create(FOwner);
  FA_BuSzPts := Result;
end;
...
begin
  Result := TCT_TextBulletTypefaceFollowText.Create(FOwner);
  FA_BuFontTx := Result;
end;

function  TEG_TextBulletTypeface.Create_A_BuFont: TCT_TextFont;
begin
  Result := TCT_TextFont.Create(FOwner);
  FA_BuFont := Result;
end;
...
begin
  Result := TCT_TextTabStopXpgList.Create(FOwner);
  FA_TabXpgList := Result;
end;
...
begin
  Result := TCT_GeomGuideXpgList.Create(FOwner);
  FA_GdXpgList := Result;
end;
...
begin
  Result := TCT_XYAdjustHandleXpgList.Create(FOwner);
  FA_AhXYXpgList := Result;
end;

function  TCT_AdjustHandleList.Create_A_AhPolarXpgList: TCT_PolarAdjustHandleXpgList;
begin
  Result := TCT_PolarAdjustHandleXpgList.Create(FOwner);
  FA_AhPolarXpgList := Result;
end;
...
begin
  Result := TCT_ConnectionSiteXpgList.Create(FOwner);
  FA_CxnXpgList := Result;
end;
...
begin
  Result := TCT_Path2DXpgList.Create(FOwner);
  FA_PathXpgList := Result;
end;
...
begin
  Result := TCT_SphereCoords.Create(FOwner);
  FA_Rot := Result;
end;
...
begin
  Result := TCT_Point3D.Create(FOwner);
  FA_Anchor := Result;
end;

function  TCT_Backdrop.Create_A_Norm: TCT_Vector3D;
begin
  Result := TCT_Vector3D.Create(FOwner);
  FA_Norm := Result;
end;
...
begin
  ...
  if FA_BevelT <> Nil then 
    Inc(ElemsAssigned,FA_BevelT.CheckAssigned);
  if FA_BevelB <> Nil then 
    Inc(ElemsAssigned,FA_BevelB.CheckAssigned);
...
begin
  Result := TCT_Bevel.Create(FOwner);
  FA_BevelT := Result;
end;
...
begin
  Result := TCT_TextCharacterProperties.Create(FOwner);
  FA_RPr := Result;
end;

function  TCT_TextField.Create_A_PPr: TCT_TextParagraphProperties;
begin
  Result := TCT_TextParagraphProperties.Create(FOwner);
  FA_PPr := Result;
end;
...
begin
  Result := TCT_GeomGuideList.Create(FOwner);
  FA_AvLst := Result;
end;
...
begin
  Result := TCT_TextNoAutofit.Create(FOwner);
  FA_NoAutofit := Result;
end;

function  TEG_TextAutofit.Create_A_NormAutofit: TCT_TextNormalAutofit;
begin
  Result := TCT_TextNormalAutofit.Create(FOwner);
  FA_NormAutofit := Result;
end;

function  TEG_TextAutofit.Create_A_SpAutoFit: TCT_TextShapeAutofit;
begin
  Result := TCT_TextShapeAutofit.Create(FOwner);
  FA_SpAutoFit := Result;
end;
...
begin
  Result := TCT_Shape3D.Create(FOwner);
  FA_Sp3d := Result;
end;

function  TEG_Text3D.Create_A_FlatTx: TCT_FlatText;
begin
  Result := TCT_FlatText.Create(FOwner);
  FA_FlatTx := Result;
end;
...
begin
  Result := TCT_PresetTextShape.Create(FOwner);
  FA_PrstTxWarp := Result;
end;

function  TCT_TextBodyProperties.Create_A_Scene3d: TCT_Scene3D;
begin
  Result := TCT_Scene3D.Create(FOwner);
  FA_Scene3d := Result;
end;
...
begin
  Result := TCT_StyleMatrixReference.Create(FOwner);
  FA_LnRef := Result;
end;
...
begin
  Result := TCT_FontReference.Create(FOwner);
  FA_FontRef := Result;
end;
...
function TCT_TextBody.AsPlainText: AxUCString;
var
  i,j: integer;
begin
  Result := '';
...
function  TCT_TextBody.Create_PXpgList: TCT_TextParagraphXpgList;
begin
  if FA_PXpgList = Nil then
    FA_PXpgList := TCT_TextParagraphXpgList.Create(FOwner);
  Result := FA_PXpgList;
...
begin
  Result := TCT_GroupTransform2D.Create(FOwner);
  FA_Xfrm := Result;
end;
...
begin
  if (FNvSpPr <> Nil) and FNvSpPr.Assigned then 
    ...
    begin
      AWriter.BeginTag('xdr:nvSpPr');
      FNvSpPr.Write(AWriter);
      AWriter.EndTag;
    end
    else 
      AWriter.SimpleTag('xdr:nvSpPr')
  else 
    AWriter.SimpleTag('xdr:nvSpPr');
  ...
  if (FTxBody <> Nil) and FTxBody.Assigned then 
    ...
    begin
      AWriter.BeginTag('xdr:txBody');
      FTxBody.Write(AWriter);
      AWriter.EndTag;
    end
    else 
      AWriter.SimpleTag('xdr:txBody');
end;
...
begin
  Result := TCT_SupplementalFontXpgList.Create(FOwner);
  FA_FontXpgList := Result;
end;
...
begin
  Result := TCT_LinePropertiesXpgList.Create(FOwner);
  FA_LnXpgList := Result;
end;
...
begin
  Result := TCT_EffectStyleItemXpgList.Create(FOwner);
  FA_EffectStyleXpgList := Result;
end;
...
begin
  Result := TCT_FontCollection.Create(FOwner);
  FA_MajorFont := Result;
end;
...
begin
  Result := TCT_FillStyleList.Create(FOwner);
  FA_FillStyleLst := Result;
end;

function  TCT_StyleMatrix.Create_A_LnStyleLst: TCT_LineStyleList;
begin
  Result := TCT_LineStyleList.Create(FOwner);
  FA_LnStyleLst := Result;
end;

function  TCT_StyleMatrix.Create_A_EffectStyleLst: TCT_EffectStyleList;
begin
  Result := TCT_EffectStyleList.Create(FOwner);
  FA_EffectStyleLst := Result;
end;

function  TCT_StyleMatrix.Create_A_BgFillStyleLst: TCT_BackgroundFillStyleList;
begin
  Result := TCT_BackgroundFillStyleList.Create(FOwner);
  FA_BgFillStyleLst := Result;
end;
...
begin
  Result := TCT_CustomGeometry2D.Create(FOwner);
  FA_CustGeom := Result;
end;
...
begin
  Result := TCT_Ratio.Create(FOwner);
  FA_Sx := Result;
end;
...
begin
  Result := TCT_CustomColorXpgList.Create(FOwner);
  FA_CustClrXpgList := Result;
end;
...
begin
  ...
  if FA_SpLocks <> Nil then
    Inc(ElemsAssigned,FA_SpLocks.CheckAssigned);
  if FA_ExtLst <> Nil then
    Inc(ElemsAssigned,FA_ExtLst.CheckAssigned);
  Result := 0;
  if ElemsAssigned > 0 then
...
begin
  Result := TCT_ShapeLocking.Create(FOwner);
  FA_SpLocks := Result;
end;
...
begin
  ...
  if (FCNvSpPr <> Nil) and FCNvSpPr.Assigned then begin
    ...
    if xaElements in FCNvSpPr.FAssigneds then begin
      AWriter.BeginTag('xdr:cNvSpPr');
      FCNvSpPr.Write(AWriter);
      AWriter.EndTag;
    end
    else
      AWriter.SimpleTag('xdr:cNvSpPr');
  end
  else
    AWriter.SimpleTag('xdr:cNvSpPr');  // TODO Must be written
end;
...
begin
  Result := TCT_NonVisualDrawingShapeProps.Create(FOwner);
  FCNvSpPr := Result;
end;
