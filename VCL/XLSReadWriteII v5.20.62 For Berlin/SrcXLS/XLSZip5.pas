    ...
    repeat
      ...
      if P > 0 then begin
        ...
        if Find(INCN) < 0 then begin
          ...
          raise Exception.Create('Duplicate filename in Zip');
      end;
    until NoMore;
    Result := True;
  finally
...
begin
  ...
  if Find(ItemName) >= 0 then
    raise Exception.CreateFmt('Duplicate file "%s" in Zip',[ItemName]);
...
begin
  ...
  FStream.Seek(FCurrZip.FCentralDirectoryFile.RelativeOffsetOfLocalHeader,soFromBeginning); 
...
begin
  if FCurrZip <> Nil then
    raise Exception.Create('There is an active zip');
...
begin
  ...
  if  (Byte_[0] = $50) and (Byte_[1] = $4B) and (Byte_[2] = $03) and (Byte_[3] = $04) then begin
    ...
    raise Exception.Create('Unknown zip file');
end;
...
begin
  if FindCentralDirectory then begin
    if ParseCentralHeaders then begin
      ...
      raise Exception.Create('File is not a valid ZIP file');
  end
  else begin
...
begin
  if FDStream <> Nil then begin
    ...
    raise Exception.Create('ZIP file is not open');
end;
...
begin
  raise Exception.Create('Not implemented, write');
end;
