...
public
     function  Add: TCT_TwoCellAnchor;
     function  CheckAssigned: integer;
     procedure Write(AWriter: TXpgWriteXML; AName: AxUCString);
  ...
  if AReader.QName = 'c:chart' then begin
    for i := 0 to AReader.Attributes.Count - 1 do begin
      if AReader.Attributes[i] = 'r:id' then begin
        FChart := TXPGDocXLSXChart.Create;
        ...
        Stream := FOwner.GrManager.XLSOPC.ReadChart(AReader.Attributes.Values[i]);
        try
...
begin
  if FChart <> Nil then begin
    ...
    try
      ...
      OPC := FOwner.GrManager.XLSOPC.CreateChart;
      ...
      AWriter.AddAttribute('r:id',OPC.Id);
      AWriter.SimpleTag('c:chart');

      if FChart.Style <> Nil then begin
        Stream.Clear;
        FChart.Style.SaveToStream(Stream);
        OPC2 := FOwner.GrManager.XLSOPC.CreateChartStyle(OPC);
        FOwner.GrManager.XLSOPC.ItemWrite(OPC2,Stream);
        FOwner.GrManager.XLSOPC.ItemClose(OPC2);
      end;
      if FChart.Colors <> Nil then begin
        Stream.Clear;
        FChart.Colors.SaveToStream(Stream);
        OPC2 := FOwner.GrManager.XLSOPC.CreateChartColors(OPC);
        FOwner.GrManager.XLSOPC.ItemWrite(OPC2,Stream);
        FOwner.GrManager.XLSOPC.ItemClose(OPC2);
      end;
    finally
...
begin
  ...
  if FA_GrpSpLocks <> Nil then 
    Inc(ElemsAssigned,FA_GrpSpLocks.CheckAssigned);
  if FA_ExtLst <> Nil then 
    Inc(ElemsAssigned,FA_ExtLst.CheckAssigned);
  Result := 0;
  if ElemsAssigned > 0 then 
...
begin
  case AReader.QNameHashB of
    $13D59EEE: begin
      if FFrom = Nil then 
        FFrom := TCT_Marker.Create(FOwner);
      Result := FFrom;
    end;
    $6018D3C3: begin
      if FExt = Nil then 
...
function  TCT_OneCellAnchor.Create_From: TCT_Marker;
begin
...
procedure TXPGDocXLSXDrawing.LoadFromStream(AStream: TStream);
begin
...
procedure TXPGDocXLSXDrawing.SaveToStream(AStream: TStream);
begin
