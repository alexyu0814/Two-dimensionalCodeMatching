...
public
     ...
     function  ReadVmlDrawing(AId: AxUCString): TStream;
     ...
     function  CreateWorkbook: TOPCItem;
  ...
  try
    if FindByType(ATypeName,List) then begin
      if List.Count > 1 then
        raise Exception.CreateFmt('OPC: only one item of type %s excpected',[ATypeName]);
      Result := List[0];
    end;
  finally
    List.Free;
...
begin
  ...
  if i < 0 then
    Exit;
//    raise Exception.CreateFmt('Can not find OPC item "%s"',[Filename]);
...
begin
  if not (AItem.FState in [opcsRead,opcsWrite]) then
    raise Exception.Create('Close on not open OPC Item.');
...
begin
  if AItem.State <> opcsWrite then
    raise Exception.Create('OPC Item is not opened for writing.');
...
begin
  ...
  if i >= 0 then begin
    ...
    if Result.Type_ <> AType then
      raise Exception.CreateFmt('OPC Item is not of excpected type "%s"',[AType]);
...
begin
  if FState <> opcsClosed then
    raise Exception.Create('OPC is allready open');
...
{$else}
    raise Exception.Create('No alternate Zip defined.')
{$endif}
  else
...
begin
  ...
  if i < 0 then
    Exit;
//    raise Exception.CreateFmt('Can not find "%s" in OPC',[AItem.AbsoluteTarget]);
...
begin
  if FWorkbook = Nil then
    raise Exception.Create('OPC Workbook is not open.');
//  if FWorkbook.State <> opcsWrite then
//    raise Exception.Create('OPC Workbook is not opened for writing');
end;
...
begin
  ...
  if Result = Nil then
    raise Exception.Create('Can not find Table in OPC');
end;
...
begin
  ...
  if i < 0 then
    raise Exception.Create('Can not find worksheet in OPC');
...
begin
  ...
  if not FRoot.FindByType(OPC_XLSX_WORKBOOK,FResultList) then
    raise Exception.Create('Can not find workbook in OPC');
  if FResultList.Count > 1 then
    raise Exception.Create('More than one workbook in OPC');
...
begin
  ...
  if Item = Nil then
    raise Exception.Create('Can not find Chart in OPC');
...
begin
  ...
  if Item = Nil then
    raise Exception.Create('Can not find Image in OPC');
  Result := ReadItemData(Item);
  ATarget := Item.Target;
  ItemClose(Item);
end;
...
begin
  ...
  if OPC = Nil then
    raise Exception.CreateFmt('Can not find pivotCacheDefintion %s in OPC',[AId]);
...
begin
  ...
  if OPCOwner <> Nil then begin
    ...
    if OPC = Nil then
      raise Exception.CreateFmt('Can not find pivotCacheRecords %s in OPC',[AId]);
    Result := ReadItemData(OPC);
    ItemClose(OPC);
  end
  else
    raise Exception.CreateFmt('Can not find pivotCacheDefintion %s in OPC',[AOwnerId]);
end;
...
begin
  ...
  if OPC = Nil then
    raise Exception.CreateFmt('Can not find pivotTable %s in OPC',[AId]);
  Result := ReadItemData(OPC);
...
var
  Item: TOPCItem;
begin
  Item := ItemOpenRead(FCurrTable,OPC_XLSX_QUERYTABLE,FCurrTable.Id);
  if Item <> Nil then begin
    Result := ReadItemData(Item);
    ItemClose(Item);
  end
...
begin
  ...
  if FCurrDrawing = Nil then
    raise Exception.Create('Can not find Drawing in OPC');
...
function TOPC_XLSX.ReadVmlDrawing(AId: AxUCString): TStream;
var
  OPC: TOPCItem;
begin
  OPC := ItemOpenRead(FCurrSheet,OPC_XLSX_VMLDRAWING,AId);
  if OPC = Nil then
    raise Exception.CreateFmt('Can not find VmlDrawing %s in OPC',[AId]);
  Result := ReadItemData(OPC);
  ItemClose(OPC);
end;
...
begin
  ...
  if Item = Nil then
    raise Exception.Create('Can not find ExternalLink in OPC');
...
function TOPC_XLSX.CreateWorkbook: TOPCItem;
begin
  if FWorkbook <> Nil then
    raise Exception.Create('OPC workbook is open');
...
begin
  ...
  if not FRoot.FindByType(OPC_DOCX_DOCUMENT,FResultList) then
    raise Exception.Create('Can not find document in OPC');
  if FResultList.Count > 1 then
    raise Exception.Create('More than one document in OPC');
