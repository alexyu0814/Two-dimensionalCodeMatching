...
//                        FCurrXc12Sheet := FManager.Worksheets.Add(FManager.Worksheets.Count + 1);
                      ...
                      else   raise Exception.Create('Invalid sheet type in file');
                    end;
...
begin
  ...
  with PRecMULRK(PBuf)^ do begin
    for i := 0 to (Header.Length - 6) div 6 - 1 do begin
      if FManager.DirectRead then
        DoDirectRead(Col1 + i,Row,DecodeRK(RKs[i].RK))
      else
        FCurrXc12Sheet.Cells.StoreFloat(Col1 + i,Row,RKs[i].XF + XLS_STYLE_DEFAULT_XF_COUNT,DecodeRK(RKs[i].RK));
    end;
  end;
end;
...
begin
  ...
  raise Exception.CreateFmt('Fixup error in shared formula at C=%d,R=%d (%s)',
...
begin
  ...
  if FFormatCnt > High(FEx40NumFmts) then
    SetLength(FEx40NumFmts,FFormatCnt);
...
begin
  ...
  if FXLS.Version >= xvExcel97 then begin
    ...
    for i := 1 to PWordArray(PBuf)[0] - 1 do begin
      Brk := FCurrXc12Sheet.CustomSheetViews[0].RowBreaks.Add;
