...
begin
  if (FBufPtr + ASize + SizeOf(TAttributeData)) > FBufSize then
    ...
    raise Exception.Create('Out of memory in Encode Formula');
...
begin
  ...
  for i := 0 to FPostfixSz - 1 do begin
    ...
    case Ptgs.Id of
      ...
      else          raise Exception.CreateFmt('Unknown ptgs "%.2X"',[Ptgs.Id]);
    end;
  end;
...
begin
  case SheetTokens of
    tsXS: begin
      ...
      if S <> '0' then
        raise Exception.CreateFmt('Unknown workbook "%s" name',[S]);
      S := TokenAsString(FTokens[AIndex]);
{$ifdef XLSRWII}
...
{$endif}
    ...
    else raise Exception.Create('Unknown name reference');
  end;
...
//      else
  ...
  case Token.TokenType of
    ...
    xttR1C1AbsRow   : ;
    xttName         : SaveName;
    else raise Exception.CreateFmt('Unknown sheet reference type: "%.2X"',[Integer(Token.TokenType)]);
  end;
end;
...
begin
  case APtgs.Id of
    ...
    else raise Exception.CreateFmt('Unknown ptg: %d',[APtgs.Id]);
  end;
end;
...
{$endif}
      ...
      if FStopOnError then begin
        ...
        raise Exception.CreateFmt('Invalid xtt token %d',[Integer(AToken.TokenType)]);
    end;
  end;
...
begin
  case APtgs.Id of
    ...
    xptgOpEQ      : Result := 30;
    xptgOpNE      : Result := 30;
    else          raise Exception.CreateFmt('Unknown operator %.2X',[APtgs.Id]);
  end;
end;
...
begin
  case APtgs.Id of
    ...
    else raise Exception.CreateFmt('Unknown ptgs %d',[Aptgs.Id]);
  end;
end;
