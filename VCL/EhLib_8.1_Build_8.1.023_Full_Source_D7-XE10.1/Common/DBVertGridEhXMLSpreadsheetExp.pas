...
var
    ...
    begin
      ...
      begin
        ...
        CellType  := 'String';
              ...
              and (not FieldRow.Field.IsNull) // check for minimum date
              and (FieldRow.Field.AsDateTime < EncodeDate(1900, 1, 1)) // if date < 01-01-1900 then write as text
      then
      begin
        CellValue := RowCellParamsEh.Text;
        CellType  := 'String';
  ...
  begin
    ...
    with RowCellParamsEh do
    begin
      Row := -1;
      Col := -1;
      State := [];
      Font := DBVertGridEh.Canvas.Font;
      Background := FieldRow.Color;
      NewBackground := FieldRow.Color;
      Alignment := FieldRow.Alignment;
      ImageIndex := FieldRow.GetImageIndex;
      Text := FieldRow.DisplayText;
      CheckboxState := FieldRow.CheckboxState;

      if Assigned(DBVertGridEhCrack.OnGetCellParams) then
        DBVertGridEhCrack.OnGetCellParams(DBVertGridEh, FieldRow, Font, NewBackground, State);
