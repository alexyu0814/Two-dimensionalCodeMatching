...
uses Math, ComObj, StrUtils, ClipBrd,
{$IFDEF FPC}
{$ELSE}
  PrintPivotGridsEh,
{$ENDIF}
...
begin
  ...
  SliceDisplayNames[dtslNonEh] := SPivotSliceNonDisplayNameEh;
  SliceDisplayNames[dtslYearEh] := SPivotSliceYearDisplayNameEh;
  SliceDisplayNames[dtslQuarterEh] := SPivotSliceQuarterDisplayNameEh;
  SliceDisplayNames[dtslMonthEh] := SPivotSliceMonthDisplayNameEh;
  SliceDisplayNames[dtslWeekEh] := SPivotSliceWeekDisplayNameEh;
  SliceDisplayNames[dtslDayEh] := SPivotSliceDayDisplayNameEh;
  SliceDisplayNames[dtslHourEh] := SPivotSliceHourDisplayNameEh;
  SliceDisplayNames[dtslMinEh] := SPivotSliceMinDisplayNameEh;
  SliceDisplayNames[dtslSecEh] := SPivotSliceSecDisplayNameEh;
  SliceDisplayNames[dtslMSecEh] := SPivotSliceMSecDisplayNameEh;

  PivotAggrValueDisplayNames[svtNonEh] := SPivotSumFunctionNonEh;
  PivotAggrValueDisplayNames[svtSumEh] := SPivotSumFunctionSumEh;
  PivotAggrValueDisplayNames[svtCountEh] := SPivotSumFunctionCountEh;
  PivotAggrValueDisplayNames[svtAvgEh] := SPivotSumFunctiontAvgEh;
  PivotAggrValueDisplayNames[svtMaxEh] := SPivotSumFunctionMaxEh;
  PivotAggrValueDisplayNames[svtMinEh] := SPivotSumFunctionMinEh;
  PivotAggrValueDisplayNames[svtCountDistinctEh] := SPivotSumFunctionCountDistinctEh;
  PivotAggrValueDisplayNames[svtCustomEh] := SPivotSumFunctionCustomEh;
...
// DisplayValue := '';
  if not DataSourceIsEmpty and
    ...
    (PivotCel.VertAggrLevelCol = ActualRowFlds.Count-1)
  then
    ADrawCellParams.DisplayValue := SGrandTotalEh
  else if not DataSourceIsEmpty and
          FShowGrandTotalCols and
          (ACol = FullColCount-1*ActualValueFields.Count) and
          (ARow = ActualColFlds.Count)
  ...
  begin
    ...
    if ((PivotCel.VertAggrLevelRow > 0) or (PivotCel.HorzAggrLevelCol > 0)) and
       (ADrawCellParams.DisplayValue <> '')
    then
      ADrawCellParams.DisplayValue := ADrawCellParams.DisplayValue + ' ' + STotalEh;
  end;
end;
...
begin
  ...
  FProgressPanel.FLabelTimePassed.Caption := SElapsedTimeEh + ' - ' + FormatDateTime('NN:SS', ElapsedTime / 86400000);
...
begin
  PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, FPercent);
end;
...
begin
  ...
  if (FNotificationConsumers <> nil) and ((GetTickCount - FCheckCancelRequestTime >= 300) or (Percent = 100))
  then
  begin
    for I := 0 to FNotificationConsumers.Count - 1 do
    begin
    ...
    FCheckCancelRequestTime := GetTickCount;
...
begin
  FBuildDataProgressTicks := GetTickCount;
  FCheckCancelRequestTime := GetTickCount;
  ...
  LogTimeMetric('BuildGridData', GetTickCount - FBuildDataProgressTicks)
end;
...
begin
  ...
  try
    ...
    PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 1);
    FillBasePivotData;
    PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 5);
  finally
    BaseTable.EnableControls;
  end;
...
begin
  ...
  LogTimeMetric('BuildPivotData', GetTickCount - FBuildDataProgressTicks)
end;
...
begin
  ...
  PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 100);
  PivotDataChanged;
end;
...
begin
  ...
  PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 16);
  MakeColsTable;
  PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 17);

  SortFields := FActualRowFlds.CommaText  + ',' + FActualColFlds.CommaText;
  BaseTable.SortByFields(SortFields);
  PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 36);
  MakeRowsTable;
  PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 37);
...
begin
  ...
  if FActualRowFlds.Count > 0 then
    ...
    begin
      PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 0);
  ...
  begin
    ...
    NumDataField.DisplayLabel := SPivotValueFieldDisplayNameEh;
...
begin
  ...
  for i := 0 to FActualRowFlds.Count-1 do
  begin
    PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 2);
  ...
  for i := 0 to ActualValueFields.Count-1 do
  begin
    PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 3);
  ...
  begin
    ...
    for i := 0 to FActualRowFlds.Count-1 do
    begin
      PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 4);
...
begin
  ...
  begin
    ...
    for i := 0 to KeyLength-1 do
    begin
      PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 38);
  ...
  begin
    PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 39);
    ResultAggrTable.Append;
    for i := 0 to b-1 do
      ResultAggrTable.Fields[i].Value := CompValue[i];
    for k := 0 to ActualValueFields.Count-1 do
  ...
  for i := 0 to ActualValueFields.Count-1 do
  begin
    PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 40);
...
begin
  ...
  begin
    ...
    for k := 0 to ActualValueFields.Count-1 do
    begin
      PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 80);
      AggrLevelArrValue[i][k] := TAggrValueHolderEh.Create;
      AggrLevelArrValue[i][k].AggrFunc := ActualValueFields[k].SumFunction;
      AggrLevelArrValue[i][k].ResetAggrHolder;
    end;
...
begin
  ...
  for i := 0 to FActualColFlds.Count-1 do
  begin
    PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 60);
  ...
  SortFields := FActualRowFlds.CommaText + ',' + FActualColFlds.CommaText;
  BaseTable.SortByFields(SortFields);
  PivotDataChangeProgress(GetTickCount - FBuildDataProgressTicks, 99);
...
begin
  ...
  PivotDataFinishChanging;
  LogTimeMetric('SetDataFilter', GetTickCount - FBuildDataProgressTicks)
end;
...
begin
  ...
  begin
    ...
    begin
      FAggrText := FAggrText + DivText + SGridSelectionInfo_Sum + ': ' + FormatFloat(',#.####', SumVal);
      DivText := '    ';
    end;
    if not VarIsNull(CountVal) then
    begin
      FAggrText := FAggrText + DivText + SGridSelectionInfo_Cnt + ': ' + FormatFloat(',#', CountVal);
      DivText := '    ';
      if SumVal <> 0 then
    ...
    begin
      FAggrText := FAggrText + DivText + SGridSelectionInfo_Min + ': ' + FormatFloat(',#', MinVal);
      DivText := '    ';
    end;
    if not VarIsNull(MaxVal) then
    begin
      FAggrText := FAggrText + DivText + SGridSelectionInfo_Max + ': ' + FormatFloat(',#', MaxVal);
      DivText := '    ';
    end;
    if not VarIsNull(AvgVal) then
    begin
      FAggrText := FAggrText + DivText + SGridSelectionInfo_Evg + ': ' + FormatFloat(',#', AvgVal);
      DivText := '    ';
    end;
    FAggrText := FAggrText + DivText + SCellCountEh + ': ' + FloatToStr(CellCountVal);
...
//function TPivotGridCellParamsEh.ActualDataAggregateFontColor: TColor;
//begin
//
//end;
...
//  Parent := TWinControl(AOwner);
  ...
  FLabelNote.Caption := SPivotDataBuildingInProgressEh;
  FLabelNote.Left := 8;
  FLabelNote.Top := 40;
  ...
  FLabelEsc.Caption := SPressESCToCancelEh;
  FLabelEsc.Left := 155;
  FLabelEsc.Top := 60;

  FLabelTimePassed := TLabel.Create(Self);
  FLabelTimePassed.Parent := Self;
  FLabelTimePassed.Caption := SElapsedTimeEh;
...
begin
  ...
  begin
    // FlipChildren(False); { Don't work by unknown reason }
