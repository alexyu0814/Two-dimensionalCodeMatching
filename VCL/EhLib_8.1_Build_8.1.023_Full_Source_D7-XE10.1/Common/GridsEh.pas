...
{$ENDIF}
    ...
    procedure InvalidateRect(const ARect: TGridRect);
...
//    procedure DrawCellDataBackground(ACol, ARow: Integer; ARect: TRect; State: TGridDrawState); virtual;
...
////      Pen.Style := psDot;
//    Canvas.Pen.Color := StyleServices.GetSystemColor(clWindow);
    ...
    begin
      ...
      try
        ...
        begin
          ...
          Pen.Mode := pmXor;
          Pen.Width := 5;
          Pen.Color := StyleServices.GetSystemColor(clWhite);
...
begin
  ...
  begin
    ...
    Canvas.Pen.Color := 0; //Fixup XE Styles
...
//  PageHeight := 0;
  ...
  //      VK_NEXT: NewCurrent.Y := DrawInfo.Vert.LastFullVisibleCell;
      ...
      case Key of
        VK_UP: NewCurrent.Y := NextSelectableCellFor(Col, Row, NewCurrent.X, NewCurrent.Y-1).Y;
          //Dec(NewCurrent.Y);
        VK_DOWN: NewCurrent.Y := NextSelectableCellFor(Col, Row, NewCurrent.X, NewCurrent.Y+1).Y;
          //Inc(NewCurrent.Y);
        VK_LEFT:
...
//    OutputDebugString('SetGridTimer Enabled = False');
//    FGridTimer.Interval := Interval;
  end else
  begin
    if Interval = 0
    ...
    FGridTimer.Enabled := Enabled;
//    OutputDebugString(LPCWSTR('SetGridTimer Enabled = True Interval = ' + IntToStr(Interval)));
  end;
end;
...
{$IFDEF EH_LIB_9}
  if csDesigning in ComponentState
    then Result := False
    else Result := inherited CanFocus;
{$ELSE}
...
//    Canvas.TextOut(ARect.Left + 2, ARect.Top, s);
    WriteTextEh(Canvas, ARect, False, 2, 0, s, taLeftJustify, tlTop, False, False, 0, 0, UseRightToLeftAlignment, True);
  end else
  begin
