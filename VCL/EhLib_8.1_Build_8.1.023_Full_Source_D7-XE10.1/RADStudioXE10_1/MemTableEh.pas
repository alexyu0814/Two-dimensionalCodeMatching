...
{                     Build 8.1.048                     }
{                                                       }
{      Copyright (c) 2004-16 by Dmitry V. Bolshakov     }
{                                                       }
{*******************************************************}
...
begin
//  bmd.MemRec := MemRec;
//  bmd.RecViewIndex := RecordNumber;
//{$IFDEF EH_LIB_12}
//  SetLength(Result, SizeOf(TBookmarkDataEh));
//  Move(MemRec, Result[0], SizeOf(TBookmarkDataEh));
//{$ELSE}
//  Move(MemRec, Result^, SizeOf(TBookmarkDataEh));
//{$ENDIF}
  Result := nil;
end;
...
begin
  ...
  begin
    BookmarkData.MemRec := nil;
    BookmarkData.RecViewIndex := -1;
...
begin
  if FInstantReadMode and not IsForWrite then
    ...
    case State of
      ...
      dsInternalCalc: RecBuf := BufferToRecBuf(TRecordBuffer(InstantBuffer));
      else
        RecBuf := nil;
    end;
  Result := RecBuf <> nil;
  ...
  case Field.DataType of
    ...
    ftBCD:
      if NativeFormat
        ...
        else CurrencyToBytes(Value, Buffer);
    ftBoolean:
      WordBoolToBytes(Value, Buffer);
    ftDate, ftTime, ftDateTime:
      if NativeFormat
        then DataConvert(Field, @TVarData(Value).VDate, Buffer, True)
        else DoubleToBytes(Value, Buffer);
    ftBytes, ftVarBytes:
      if NativeFormat
        then DataConvert(Field, @Value, Buffer, True)
        else VariantToBytes(Value, Buffer);
    ftInterface:
      begin
...
//        WStrCopy(PWideChar(Data), PWideChar(VarToWideStr(Buffer)));
      ...
      ftDate, ftTime, ftDateTime:
        if NativeFormat
          then DataConvert(Field, Buffer, @TVarData(Data).VDate, False)
          else Data := BytesToDouble(Buffer);
      ftBCD:
        if NativeFormat
          then DataConvert(Field, Buffer, @TVarData(Data).VCurrency, False)
          else Data := BytesToCurrency(Buffer);
      ftBytes, ftVarBytes:
        if NativeFormat
          then DataConvert(Field, Buffer, @Data, False)
...
begin
  ...
  try
    ...
    BufferToRecBuf(TRecordBuffer(TempBuffer)).NeedUpdateCalcFields := True;
    BufferToRecBuf(TRecordBuffer(TempBuffer)).RecordNumber := FRecordsView.IndexOfRec(Item);
    try
      GetCalcFields(TempBuffer);
    finally
      //BufferToRecBuf(TRecordBuffer(TempBuffer)).NeedUpdateCalcFields := False;
...
begin
  ...
  begin
    ...
    if pbmd1.RecViewIndex > pbmd2.RecViewIndex then
      Result := 1
    else if pbmd1.RecViewIndex < pbmd2.RecViewIndex then
      Result := -1
    else Result := 0;
  end;
end;
...
//  IntegerToBytes(BufferToRecBuf(Buffer).Bookmark, Data);
end;
{$ELSE}
...
var
  bmd: TBookmarkDataEh;
begin
//  BufferToRecBuf(Buffer).Bookmark := BytesToInteger(Data);
...
//      FRecordsView.MemTableData.RecordsList.CachedUpdates := False;
      ...
      ARec.Edit;
      SetMemoryRecordData(TRecordBuffer(ActiveBuffer), ARec);
      ARec.Post;
//      FRecordsView.MemTableData.RecordsList.CachedUpdates := True;
      if not CachedUpdates then
        InternalApplyUpdates(FRecordsView.MemTableData, -1);
...
//  InternalRefreshFilter;
  ...
  if (ExternalMemData = nil) and InternalCalcFields
    then FRecordsView.MemTableData.OnFieldCalculation := CalcInternalFieldProg
    else FRecordsView.MemTableData.OnFieldCalculation := nil;
end;
...
//      BufferToRecBuf(InstantBuffer).NeedUpdateCalcFields := False;
    finally
      InstantReadLeave;
    end;
  finally
...
{$IFDEF EH_LIB_12}
procedure TCustomMemTableEh.RefreshInternalCalcFields(Buffer: TRecordBuffer);
{$ELSE}
...
begin
  ...
  begin
    ...
    begin
      ...
      FMTViewDataEventInactiveCount := 0;
      if (FStateInsert = True) and (FStateInsertRowNum >= RowNum) then
