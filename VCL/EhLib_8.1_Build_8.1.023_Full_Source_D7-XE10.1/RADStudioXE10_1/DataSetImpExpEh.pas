...
begin
  ...
  BinToHex(Pointer(Buffer), PChar(Result), AStream.Size);
end;
...
begin
  ...
  begin
    ...
    if FieldsMapItem.Field.DataType in [ftFloat, ftCurrency, ftBCD] then
    begin
...
var
  ...
  dsField: TField;
  SkipField: Boolean;
//found_fmi: TImportFieldsMapItemEh;
  fmi: TImportFieldsMapItemEh;
  i: Integer;
  j: Integer;
  ParsedValues: TStringList;
...
begin
  ...
  try
    BinaryStream.Size := Length(HexString) div 2;
    if BinaryStream.Size > 0 then
    begin
      HexToBin(PChar(HexString), BinaryStream.Memory, BinaryStream.Size);
      Result := VarArrayCreate([0, BinaryStream.Size-1], varByte);
      PData := VarArrayLock(Result);
      try
        BinaryStream.ReadBuffer(PData^, BinaryStream.Size);
      finally
...
begin
  ...
  try
    ...
    for i := 0 to Fields.Count-1 do
      with Add do
      begin
        DataSetFieldName := Fields[i].FieldName;
        FileFieldName := Fields[i].FieldName;
      end;
  finally
    EndUpdate;
  end;
