...
//  FRichEdit.DoSetMaxLength($7FFFFFF0);
end;
...
//        VPrinter_Canvas.FillRect(ARect);
  ...
  begin
    ...
    Value := FooterColumnItem.ConvertToDisplayText(FooterNode.FooterValues[CurColumn.Index]);
    VPrinter_Canvas.Font := FooterColumnItem.Font;
...
{$ENDIF}
    ...
    begin
      Range.hdc := VPrinter.Canvas.Handle; // Canvas.Handle;
...
{$ENDIF}
      ...
      SendMessage(Strings.FRichEdit.Handle, EM_FORMATRANGE, 0, 0); // flush buffer
      try
        repeat
          Range.rc := SaveRect;
...
{ TODO  : When printer is not installed we get error. }
          Range.hdcTarget := VPrinter.Handle; //hdc;
...
begin
  ...
  try
    ...
    if FSubstitutesNames <> nil then //Substitution
    begin
...
var
  rest: TRichEditStrings;
begin
//  Result := -1;
  rest := TRichEditStrings(RichStrings);
  repeat
    Result := rest.FRichEdit.FindText(SearchStr, StartPos, Length, Options);
    if Result <> -1 then
    begin
      rest.FRichEdit.SelStart := Result;
{$IFDEF CIL}
      rest.FRichEdit.SelLength := Borland.Delphi.System.Length(SearchStr);
{$ELSE}
      rest.FRichEdit.SelLength := System.Length(SearchStr);
{$ENDIF}
      rest.FRichEdit.SelText := ReplaceStr;
    end;
