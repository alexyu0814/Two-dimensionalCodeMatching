...
begin
  ...
  begin
    ...
    with Result do
{$IFDEF DESIGNTIME}
      if (Designer = ADesigner) and (MasterComponent = AMasterComponent) then
{$ENDIF}
      begin
        Show;
        BringToFront;
...
{$IFDEF DESIGNTIME}
    Designer := ADesigner;
{$ENDIF}
    MasterComponent := AMasterComponent;
    Show;
  except
    Free;
  end;
...
begin
  ...
  if ChildrenDesignServiceList = nil then
    ChildrenDesignServiceList := TList.Create;
...
//    ShowMessage('TCompChildrenEditor.DataSource1DataChange Designer.SelectComponent(Child);');
{$ENDIF}
  end;
  finally
    FInSelection := False;
...
procedure TCompChildrenEditor.FormKeyDown(Sender: TObject; var Key: Word;
  Shift: TShiftState);
begin
//  if Key = VK_F11 then
//  begin
//    ShowObjectInspectorForm(MemTableEh1RefComp.Value,
//      Rect(Left+Width+10, Top, Left+Width+10+300, Top+Height));
//  end;
end;
...
//  ShowMessage('ItemDeleted(const ADesigner: IDesigner;');
end;
