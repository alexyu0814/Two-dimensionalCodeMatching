...
{                      Build 8.1.183                    }
{                                                       }
{   Copyright (c) 2012-2016 by Dmitry V. Bolshakov      }
{                                                       }
{*******************************************************}
...
{$ENDIF}
  ...
  {$ENDIF}
    ...
    function GetTextValue(IsDisplaText: Boolean): String; virtual;
...
begin
  ...
  begin
    ...
    begin
        ...
        {$ELSE}
          if not TryStrToDateTime(Text, ADateTime) then
            FPopupMonthCalendar.Date := TDate(Date)
          else
            FPopupMonthCalendar.Date := ADateTime;
        except
          if AxisBar.Field.AsDateTime = 0 then
            FPopupMonthCalendar.Date := TDate(Date)
          else
...
{$ENDIF}
          begin
            ...
            if (CharCode <> 0) and (Message.Msg = wm_Char) and
              ...
              and (AColumn <> nil) and AColumn.AutoDropDown then
            begin
...
begin
  ...
  {$ENDIF}
  mrc.Sorted := True;
  mrc.Parent := Self; // Already set parent in TPopupListboxEh.CreateWnd
...
begin
  ...
  begin
    ...
    begin
      MRUList.Rows := TPopupListboxEh(MRUListControl).ResizedRowCount;
      MRUList.Width := TPopupListboxEh(MRUListControl).ResizedWidth;
    end;
...
var
  ...
  mlc: TPopupListboxEh;
begin
  mlc := TPopupListboxEh(MRUListControl);
//  with TPopupListboxEh(MRUListControl) do
  begin
...
//    FilterMRUItems(MRUList.Items, Items);
    ...
    begin
      ...
      SetWindowPos(mlc.Handle, HWND_TOP {MOST}, P.X, P.Y, 0, 0,
        SWP_NOSIZE or SWP_NOACTIVATE or SWP_SHOWWINDOW);
    ...
    begin
      ...
      mlc.Font := Self.Font;
      mlc.RowCount := mlc.RowCount; //Reset height
      mlc.ItemIndex := -1;
      ...
      SetWindowPos(mlc.Handle, HWND_TOP {MOST}, P.X, P.Y, 0, 0,
        SWP_NOSIZE or SWP_NOACTIVATE or SWP_SHOWWINDOW);
      mlc.Visible := True; //commment for Tab key
...
//    begin
////      Result := LookupParams.KeyFields Field.Value <> Null;
//      Result := VarEquals(Grid.DataLink.DataSet.FieldValues[LookupParams.KeyFieldNames],
//                          Null)
//    end;
//  end;
...
function TAxisBarEh.GetTextValue(IsDisplaText: Boolean): String;
begin
  if IsDisplaText
...
var
  ...
  begin
    ...
    begin
      Result := AxisBar.{FullListDataSet}
        UsedLookupDataSet.Locate(LookupParams.LookupKeyFieldNames,
...
begin
  ...
  begin
    ...
    FBlankCell := False;
    FTextIsLink := CellDataIsLink;
    FImageIsLink := CellDataIsLink;
  end;
end;
...
begin
  ...
  begin
    ...
    begin
      ...
      DrawGraphicCell(Canvas, AxisBar, AreaRect, Canvas.Brush.Color, not IsCellFilled);
  end;
...
begin
  if (InplaceEditor <> nil) then
    TDBAxisGridInplaceEdit(InplaceEditor).FReadOnlyStored := False;
...
//        (StringsLocate(AxisBar.PickList, FEditText, [loCaseInsensitive]) = -1) then
    ...
    begin
      ...
      (*KeyIndex := AxisBar.KeyList.IndexOf(VarToStr(FEditKeyValue));
...
begin
//  AxisBarCrack := TAxisBarEhCrack(AxisBar);
  ...
  begin
    ...
    ABoundRect := Rect(2, 2, ColWidths[ACol], RowHeights[ARow]);
  ...
  begin
    ABoundRect := Rect(0, 2, ColWidths[ACol]-2, RowHeights[ARow]-2);
    OffsetRect(ATextRec, ABoundRect.Right - ATextRec.Right - 2, 2);
  end else //taCenter
  begin
    ABoundRect := Rect(0, 2, ColWidths[ACol], RowHeights[ARow]-2);
...
//
  ...
  begin
    OffsetRect(ATextRec, 0, RowHeights[ARow] - ATextRec.Bottom);
  end else // tlCenter
  begin
    ABoundRect := Rect(0, 0, ColWidths[ACol], RowHeights[ARow]);
...
procedure TCustomDBAxisGridEh.UpdateHotTackInfo(X, Y: Integer);
var
...
begin
  if Supports(FOwner, ILookupGridOwner, LookupGridOwner) then
    TPopupDataGridEh(LookupGridOwner.GetLookupGrid).RowHeight := Value;
  FRowHeight := Value;
end;
...
begin
  if Supports(FOwner, ILookupGridOwner, LookupGridOwner) then
    TPopupDataGridEh(LookupGridOwner.GetLookupGrid).RowLines := Value;
  FRowLines := Value;
end;
