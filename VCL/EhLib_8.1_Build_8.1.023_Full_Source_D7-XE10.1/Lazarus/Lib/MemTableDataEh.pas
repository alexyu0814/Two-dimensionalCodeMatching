...
procedure DecodeISO8601DateTime(const AISODateTime: String;
  ...
  var AMonth, ADay, AHour, AMinute, ASecond: Word; var AMillisecond: LongWord;
...
{$ELSE}
      ...
      if (NeedVarType = varSQLTimeStamp) and
         (
{$IFDEF EH_LIB_12}
...
begin
  Result := True;
//  Result := (StringDataType <> fdtGuidEh);
end;
...
//          TMemoryRecordEh(DeltaList[i]).InernalMergeChanges(True);
    end;
    FreeDeletedRecords;
    PackRecList;
    DeltaList.Clear;
    ...
    begin
      ...
      case TCANOperator(NodeOp) of
        coEQ:
          ...
          end else if (VarType(Result_Left) = varDate) and (VarType(Result_Left) = varDate) then
            Result := TimeStampsEqual(DateTimeToTimeStamp(Result_Left), DateTimeToTimeStamp(Result_Right))
          else
