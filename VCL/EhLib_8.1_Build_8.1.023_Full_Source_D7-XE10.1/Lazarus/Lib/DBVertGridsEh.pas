...
function TFieldRowEh.GetTextValue(IsDisplaText: Boolean): String;
begin
  ...
  if TMessage(Msg).Msg = wm_RButtonUp then
    ...
    begin
      ...
      begin
        ...
        ScrPnt := ClientToScreen(SmallPointToPoint(Pos));
...
//      FieldRow.RowLabel.Caption, FieldRow.RowLabel.Alignment, tlCenter,
//      RowsDefValues.RowLabel.WordWrap, FieldRow.RowLabel.EndEllipsis, 0, 0,
//      FieldRow.UseRightToLeftReading, not RowsDefValues.RowLabel.WordWrap);
  end else
  begin
    ...
    begin
      ...
      try
        ...
        if (Button = mbLeft) and
          (((X = OldCol) and (Y = OldRow)) or (dgvhAlwaysShowEditor in Options)) and
           not MouseCellIsLink
        then
      ...
      begin
        if Destination > Origin then
          if DestNode.Expanded then
            ...
            Destination := Destination; //It is Ok.
      end;
    end;
  end;
end;
...
procedure TCustomDBVertGridEh.UpdateHotTackInfo(X, Y: Integer);
var
  FieldRowIndex: Integer;
begin
...
begin
//  Result := FGrid.TabStop and FGrid.Visible and FGrid.Enabled;
